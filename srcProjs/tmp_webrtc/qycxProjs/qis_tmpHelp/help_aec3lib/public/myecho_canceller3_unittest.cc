...
//
//
int mic_myPopulateInputFrame(size_t frame_length,
                             size_t num_bands,
                             char* mic_data,
                             size_t mic_dataLen,
                             float* const* frame) {
  if (mic_dataLen != frame_length * 2)
    return -1;
  short* input = (short*)mic_data;

  for (size_t k = 0; k < num_bands; ++k) {
    for (size_t i = 0; i < frame_length; ++i) {
      // float value = static_cast<int>(frame_index * frame_length + i) +
      // offset; frame[k][i] = (value > 0 ? 5000 * k + value : 0);
      frame[k][i] = input[i];
    }
  }
  return 0;
...
int render_myPopulateInputFrame(size_t frame_length,
                                char* render_data,
                                size_t render_dataLen,
                                float* frame) {
  if (render_dataLen != frame_length * 2) {
    return -1;
  }
  short* input = (short*)render_data;
...
int mic_getOutputFrame(size_t frame_length,
                       size_t num_bands,
                       const float* const* frame,
                       char* mic_data,
                       size_t mic_dataLen) {
  if (mic_dataLen != 2 * frame_length) {
    return -1;
  }
  short* output = (short*)mic_data;
...
class EchoCanceller3Tester {
 public:
  ...
  //
  int doAec(AEC_obj* pObj,
            char* render_data,
            unsigned int render_dataLen,
            char* mic_data,
            unsigned int mic_dataLen) {
    EchoCanceller3& aec3 = *(EchoCanceller3*)pObj->pEchoCanceller3;
    //
    aec3.AnalyzeCapture(&capture_buffer_);
    OptionalBandSplit();
    mic_myPopulateInputFrame(frame_length_, num_bands_, mic_data, mic_dataLen,
                             &capture_buffer_.split_bands(0)[0]);
    render_myPopulateInputFrame(frame_length_, render_data, render_dataLen,
                                &render_buffer_.channels()[0][0]);

    aec3.AnalyzeRender(&render_buffer_);
    aec3.ProcessCapture(&capture_buffer_, false);

    //
    mic_getOutputFrame(frame_length_, num_bands_,
                       &capture_buffer_.split_bands(0)[0], mic_data,
                       mic_dataLen);
...
int aec3lib_doTest888() {
  webrtc::EchoCanceller3Tester tt(16000);

  tt.RunCaptureTransportVerificationTest();

  //
  aec3Obj_doAec(NULL, NULL, 0, NULL, 0);
...
int initAec3Obj(int sampleRate, void** ppObj) {
  ...
  AEC_obj* pObj = NULL;
  int size = sizeof(AEC_obj);
  pObj = (AEC_obj*)malloc(size);
  if (pObj == NULL) {
    return -1;
  }
  memset(pObj, 0, size);
  ...
  //
  pObj->pEchoCanceller3Tester = new webrtc::EchoCanceller3Tester(sampleRate);
  if (pObj->pEchoCanceller3Tester == NULL)
    goto errLabel;

  pObj->pEchoCanceller3 = new webrtc::EchoCanceller3(
      webrtc::EchoCanceller3Config(), absl::nullopt,  sampleRate, 1, 1);
  if (!pObj->pEchoCanceller3)
    goto errLabel;
...
int exitAec3Obj(void** ppObj) {
  ...
  //
  AEC_obj* pObj = (AEC_obj*)*ppObj;

  if (pObj->pEchoCanceller3) {
    webrtc::EchoCanceller3* pAec3 =
        (webrtc::EchoCanceller3*)pObj->pEchoCanceller3;
    delete pAec3;
    pObj->pEchoCanceller3 = NULL;
  }

  if (pObj->pEchoCanceller3Tester) {
    webrtc::EchoCanceller3Tester* pTester =
        (webrtc::EchoCanceller3Tester*)pObj->pEchoCanceller3Tester;
    delete pTester;
    pObj->pEchoCanceller3Tester = NULL;
  }
  free(pObj);
  *ppObj = NULL;
...
int aec3Obj_doAec(void* pObjParam,
  ...
  if (!pObjParam)
    return -1;
  AEC_obj* pObj = (AEC_obj*)pObjParam;
  //
  if ((int)render_dataLen != pObj->sample_rate / 100 * 2)
    return -1;
  if ((int)mic_dataLen != pObj->sample_rate / 100 * 2)
    return -1;
  ...
  pTester->doAec(pObj, render_data, render_dataLen, mic_data, mic_dataLen);
