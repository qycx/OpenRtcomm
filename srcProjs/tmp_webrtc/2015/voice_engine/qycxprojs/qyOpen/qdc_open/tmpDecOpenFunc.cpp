 ...
 extern  "C"  int  tmpDec_setFuncs(  QDC_OBJ_INFO  *  pQdcObjInfo  )
{
	...
	//  2016/04/26
	if  (  !pQdcObjInfo->pf_qoi_getPtrProperty  )  {
		tmp_showInfo(  _T(  "qdcInitDecV failed, qoi_getPtr is null"  )  );
		return  -1;
	}
	...
	if  (  !g_pf_DEC_d3d11_devVar_new  )  {
		tmp_showInfo(  _T(  "qdcInitDecV failed, g_pf_DEC_d3d11_devVar_new is null"  )  );
		return  -1;
	}
	...
	if  (  !g_pf_DEC_d3d11_devVar_free  )  {
		tmp_showInfo(  _T(  "qdcInitDecV failed, g_pf_DEC_d3d11_devVar_free is null"  )  );
		return  -1;
	}
	...
	if  (  !g_pf_decDev_my_RenderFrame  )  {
		tmp_showInfo(  _T(  "qdcInitDecV failed, g_pf_decDev_my_RenderFrame is null"  )  );
		return  -1;
	}
	iErr  =  0;
errLabel:
