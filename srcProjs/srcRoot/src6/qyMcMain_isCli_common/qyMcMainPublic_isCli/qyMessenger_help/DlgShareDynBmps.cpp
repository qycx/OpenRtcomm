...
//
int  CDlgShareDynBmps::toShareWebcam(  int  index_pShare_mem  )
{
	 int  iErr  =  -1;
	 ...
	 int  index_obj_sel  =  index_pShare_mem  +  1;
#if  0  // 临时处理一下
	 tmpiRet  =  showDlgOpAvDev(  m_hWnd,  m_hWnd,  index_obj_sel,  &bUnresizable  );
#endif 
...
//
int  dlgShareDynBmps_toShareWebcam_func(  HWND  hDlg_shareDynBmps,  int  index_pShare_mem,  void  **  ppCapStuff,  int  iMenuId_v,  BOOL  bUnresizable  )
{
	CDlgShareDynBmps  *  pDlg  =  (  CDlgShareDynBmps  *  )CDlgShareDynBmps::FromHandlePermanent(  hDlg_shareDynBmps  );
	if  (  !pDlg  )  return  -1;

	pDlg->toShareWebcam_func(  index_pShare_mem,  ppCapStuff,  iMenuId_v,  bUnresizable  );
...
//
int  CDlgShareDynBmps::toShareWebcam_func(  int  index_pShare_mem,  void  **  ppCapStuff,   int  iMenuId_v,  BOOL  bUnresizable  )
{
	 int  iErr  =  -1;
	 ...
	 //
	 if  (  pFuncs->shareDynBmps.pf_dlgShareDynBmps_toShareDynBmp(  g_pQyMc,  m_hWnd,  &m_var,  CONST_objType_webcam,  index_pShare_mem,  ppCapStuff,  pAvCompressor,  &avProps  )  )  goto  errLabel;
...
int  CDlgShareDynBmps::reloadOnvifList()
{
...

	{
		...
		//
		if  (  bChanged  )  {
			timestamp_renew(  pCtx_thread,  &new_rulesInfo.ts_rulesInfo,_T("dlgShareDynBmps.reloadOnvifList.l1050"));
			//
			CQySyncObj  syncObj;
...
int  CDlgShareDynBmps::chkAutoOpen(  )
{
	unsigned  int  uiObjType  =  CONST_objType_webcam;
	  ...
	  for  (  i  =  0;  i  <  pShare->usCnt;  i  ++  )  {
		   ...
		   //
		   doAutoOpen_webcam(  index_pShare_mem,  &webcamInfo  );
...
//
int  CDlgShareDynBmps::doAutoOpen_webcam(  int  index_pShare_mem,  WEBCAM_info  *  p  )
{
	int  iErr  =  -1;
	...
	//
	dlgShareDynBmps_toShareWebcam_func(  m_hWnd,  index_pShare_mem,  (  void  **  )&pCapStuff,  iMenuId_v,  p->ucbUnresizable  );
...
errLabel:
 ...
 void CDlgShareDynBmps::OnDestroy()
 {
	 ...
	 old_freeCapImages(  this->m_hWnd,  m_var.idc,  &m_var.images,  old_M_GetDlgItem(  m_hWnd,  idc_dst  ),  &m_var.hDc,  _T(  "dlgShareDynBmps.OnDestroy.2105"  ));
#endif  
