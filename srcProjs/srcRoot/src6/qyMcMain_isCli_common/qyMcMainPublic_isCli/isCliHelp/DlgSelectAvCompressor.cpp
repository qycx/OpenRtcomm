...
BOOL  CDlgSelectAvCompressor::bShowInfo(  )
{
   ...
   //
...
#if  0
	DLG_TALK_var		*	pCurVar  =  (  DLG_TALK_var  *  )help_getDlgTalkVar.getVar(  m_var.hCurTalk  );
	...
	if  (  bConfSetting  )  {
		...
		// 2014/10/07
#if  0
		GetDlgItem(  IDC_CHECK_useBgWall  )->ShowWindow(  SW_SHOW  );
		GetDlgItem(  IDC_STATIC_rows  )->ShowWindow(  SW_SHOW  );
		GetDlgItem(  IDC_EDIT_rows  )->ShowWindow(  SW_SHOW  );
		GetDlgItem(  IDC_STATIC_cols  )->ShowWindow(  SW_SHOW  );
		GetDlgItem(  IDC_EDIT_cols  )->ShowWindow(  SW_SHOW  );
		GetDlgItem(  IDC_STATIC_pollingInterval  )->ShowWindow(  SW_SHOW  );
		GetDlgItem(  IDC_EDIT_pollingInterval  )->ShowWindow(  SW_SHOW  );		
#endif
		//
#if  0
		if  (  pMgrVar->av.taskInfo.bTaskExists  )  {
			if  (  pMgrVar->av.taskInfo.ucbStarter  &&  pMgrVar->av.taskInfo.ucbVideoConference  )  {}
			else  {
			}
		}
#endif
...
#if 0
		  m_buStatus_1080p_conf.ShowWindow(  SW_HIDE  );
		  m_buStatus_720p_conf.ShowWindow(  SW_HIDE  );
		  m_buStatus_576p_conf.ShowWindow(  SW_HIDE  );
		  m_buStatus_480p_conf.ShowWindow(  SW_HIDE  );
		  m_buStatus_240p_conf.ShowWindow(  SW_HIDE  );
#endif
...
BOOL CDlgSelectAvCompressor::OnInitDialog()
{
	...
	//
	SetDlgItemText(IDC_CHECK_ucb2Streams, _T("屏幕共享时双流 "));
	...
	{		
		...
		//m_var.policy.ucbShowBgWall_starter  =  talkerPolicy.ucbShowBgWall_starter;
		//
		m_var.policy.ucbConfCtrl_starter  =  talkerPolicy.ucbConfCtrl_starter;  //  2017/06/25
		}
	else  {
		if  (  pMgrVar->av.taskInfo.ucbStarter  &&  pMgrVar->av.taskInfo.ucbVideoConference  )  {
			m_var.ucbStarter  =  TRUE;
			...
			//m_var.policy.ucbShowBgWall_confMem  =  pMgrVar->m_layout.ucbShowBgWall_confMem;
		}		
	}
#endif
	m_var.ucbStarter = true;
	...
	//
#if  0
	m_var.policy.usPollingIntervalInS  =  talkerPolicy.usPollingIntervalInS;
	SetDlgItemInt(  IDC_EDIT_pollingInterval,  m_var.policy.usPollingIntervalInS  );
	m_var.policy.bgWallLayoutCfg.usRows_bg  =  talkerPolicy.bgWallLayoutCfg.usRows_bg;
	SetDlgItemInt(  IDC_EDIT_rows,  m_var.policy.bgWallLayoutCfg.usRows_bg  );
	m_var.policy.bgWallLayoutCfg.usCols_bg  =  talkerPolicy.bgWallLayoutCfg.usCols_bg;
	SetDlgItemInt(  IDC_EDIT_cols,  m_var.policy.bgWallLayoutCfg.usCols_bg  );
#endif
...
void CDlgSelectAvCompressor::OnBnClickedOk()
{
	...
	case  CONST_policyAvLevel_1080p:
		if (b60)m_var.level = CONST_policyAvLevel_1080p60;
		break;
	case  CONST_policyAvLevel_2160p:
		if (b60)m_var.level = CONST_policyAvLevel_2160p60;
		break;
	default:
		break;
	}
	...
	GetDlgItemText(IDC_EDIT_bitrate_ul, str);
				   //IDC_EDIT_bitrate_ul
	bitrate = _ttol(str);
	if (bitrate < 0)bitrate = 0;
