		   ...
		   int  stopCapture_dx(  void  *  pVideoCapDevParam  )
		   {
		     ...
		     int  exitShareMediaFile(  void  **  ppShareMediaFileParam  )
			 {
			  ...
			  int  exitShareMediaDevice(  void  **  ppShareMediaDeviceParam  )
			  {
				 ...
				 int  dyn_removeMosaicFromD3dWall(  int  iIndex_sharedObj  )
				 {
					 return  -1;
				 }


				  int  dyn_getVal_bExists_mosaic(  int  iIndex_sharedObj,  BOOL  *  pbExists  )
				  {
					  return  -1;
				  }
...
int  chkFromPlayerIndex_v(  unsigned  int  uiTransformType,  void  *  pQY_TRANSFORM,  myDRAW_VIDEO_DATA  *  pPkt,  void  *  p2  )
{
   ...
   int  doDecodeVideo_tool(  QY_TRANSFORM  *  pTransform,  int  index_pMems_from,  MIS_MSGU  *  pMsgBuf  )
   {
	   return  -1;
   }
...
BOOL  tmp_bChkQyMcEnv_mis(  void  *  pQY_MC  )
{
 ...
 extern  "C"  DWORD WINAPI mcThreadProc_decVideo( LPVOID lpParameter )
 {
  ...
  int  dec_parseEncVPkt_i(  unsigned  int  uiTransformType,  void  *  pParent_transform,  void  *  pPkt_myDRAW_VIDEO_DATA,  long  *  p_old_lPktId_alloc,  unsigned  char  **  ppInput,  unsigned  int  *puiSize,  unsigned  int  *  puiSampleTimeInMs,  unsigned  int  *  puiPts,  SMPL_bsRead_param  *  pParam  )
  {
	  return  -1;
  }
