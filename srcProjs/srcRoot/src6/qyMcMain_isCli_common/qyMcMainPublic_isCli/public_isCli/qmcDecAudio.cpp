...
//
int  transformTaskAudioPkt_useAudioQ2(  QY_TRANSFORM  *  pTransform,  MIS_MSG_TASK  *  pMsgTask,  TRANSFER_AUDIO_DATA  *  pReq,  PKT_info_toDec  *  pPktInfo_toDec,  int  index_pMems_from,  int  index_activeMems_from,  myPLAY_AUDIO_DATA	*	pPkt_dec,  MIS_MSGU  *  pMsgBuf  );
...
//  2016/12/13
#if  0
 int  getNextAPktInfo_toDec(  PKT_info_toDec  *  pCurPktInfo,  QY_Q2  *	pAudioQ2  )
{
	int  iErr  =  -1;
	MIS_MSGU  *  pMsg  =  (  MIS_MSGU  *  )pCurPktInfo->pMIS_MSGU;
	IM_CONTENTU					*	pContent					=	NULL;
...
//
#if  0
 void  clean_PKT_info_toDec(  PKT_info_toDec  *  p,  LPCTSTR  hint  )
 {
 ...
 int  doDecodeAudio_default(  QY_TRANSFORM  *  pTransform,  int  index_pMems_from1,  int  index_activeMems_from,  MIS_MSGU  *  pMsgBuf  )
{
	...
	MACRO_setMyStep(  pTransform->audio.debugStep,  21  );
		 ...
		 if  (  !pTransform->audio.ucbInited  )  
		 {
			 ...
			 qyShowInfo1(  CONST_qyShowType_qwmComm,  0,  (char*)  "",  pProcInfo->who_showInfo,  0,  _T(  ""  ),  _T(  ""  ),  _T(  "doDecodeAudio_default: initTaskAvFrom_audio %d"  ),  index_activeMems_from  );
		 }
		 if  (  !pTransform->audio.ucbInited  )  {
			 #ifdef  __DEBUG__
		 ...
		 for  (  ;  ;  )  {
			  ...
			  //
			  transformTaskAudioPkt(pTransform, &tmp_pMem, data, pPkt_dec, pMsgBuf);
 ...
 extern "C" DWORD WINAPI mcThreadProc_decAudio( LPVOID lpParameter )
{
	 ...
	 //
	 unsigned  int  uiBufSize_pkt_dec  =  pProcInfo->cfg.uiBufSize_myPlayAudioData;  //  offsetof(  myPLAY_AUDIO_DATA,  buf  )  +  pProcInfo->cfg.uiBufSize_myPlayAudioData;	//  
...
#define		CONST_mutexNamePrefix_decA_tmpDataQ		_T(  "decA_tmp"  )
	...
	pTransformAudio->pInDataCacheQ  =  (  GENERIC_Q  *  )mymalloc(  size  );
	if  (  !pTransformAudio->pInDataCacheQ  )  goto  errLabel;
	memset(  pTransformAudio->pInDataCacheQ,  0,  size  );
	size  =  sizeof(  GENERIC_Q  );
	pTransformAudio->pTmpDataQ  =  (  GENERIC_Q  *  )mymalloc(  size  );
	if  (  !pTransformAudio->pTmpDataQ  )  goto  errLabel;
	memset(  pTransformAudio->pTmpDataQ,  0,  size  );

	//
	memset(  &tmpCfg,  0,  sizeof(  tmpCfg  )  );
	_sntprintf(  tmpCfg.name,  mycountof(  tmpCfg.name  ),  _T(  "%s-%d-%d-%d"  ),  CONST_mutexNamePrefix_decA_inDataCacheQ,  iTaskId,  index_activeMems_from,  tn  );
	_sntprintf(  tmpCfg.mutexName_prefix,  mycountof(  tmpCfg.mutexName_prefix  ),  _T(  "%s-%d-%d-%d"  ),  CONST_mutexNamePrefix_decA_inDataCacheQ,  iTaskId,  index_activeMems_from,  tn  );
	...
	if  (  initGenericQ(  &qCfgEx,  pTransformAudio->pInDataCacheQ  )  )  goto  errLabel;

	//
	memset(  &tmpCfg,  0,  sizeof(  tmpCfg  )  );
	_sntprintf(  tmpCfg.name,  mycountof(  tmpCfg.name  ),  _T(  "%s-%d-%d-%d"  ),  CONST_mutexNamePrefix_decA_tmpDataQ,  iTaskId,  index_activeMems_from,  tn  );
	_sntprintf(  tmpCfg.mutexName_prefix,  mycountof(  tmpCfg.mutexName_prefix  ),  _T(  "%s-%d-%d-%d"  ),  CONST_mutexNamePrefix_decA_tmpDataQ,  iTaskId,  index_activeMems_from,  tn  );
	tmpCfg.uiMaxQNodes  =  100;
	...
	if  (  initGenericQ(  &qCfgEx,  pTransformAudio->pTmpDataQ  )  )  goto  errLabel;
#endif
