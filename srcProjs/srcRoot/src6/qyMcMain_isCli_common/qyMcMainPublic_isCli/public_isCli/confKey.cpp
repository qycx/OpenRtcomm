...
__declspec(dllexport)  int  sendTaskInteractionResp(__int64  tStartTime_req, int  uiTranNo_req, __int64 tStartTime_org, int uiTranNo_org, unsigned  char  ucStep, __int64  grpId, __int64  toId)
{
	...
	do {
		...
		req.ucStep = ucStep;
		//
		req.tStartTime_org = tStartTime_org;
		req.uiTranNo_org = uiTranNo_org;
		//req.uiContentType_org  =  m_var.av.taskInfo.uiContentType_org;
		//

		//
		req.idInfo_imGrp_related.ui64Id = grpId;
		//	
		int  lenInBytes;
		...
		//					
		//		
		QY_MESSENGER_ID  idInfo_to;  idInfo_to.ui64Id = toId;
		if (postMsgTask2Mgr_mc(pMisCnt, CONST_misMsgType_outputTask, ucFlg, usCode, tStartTime_req, uiTranNo_req, 0, 0, 0, (char*)&req, lenInBytes, NULL, NULL, NULL, &idInfo_to, 0, NULL, FALSE))
		{
			break;
		}
