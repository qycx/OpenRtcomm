 ...
 int  openLocalVideo_rtsp_step0(  MC_VAR_common  *  pProcInfoCommon,  unsigned  int  uiRtspType,  LPCTSTR  url,  int  tn_rtspCliPipe,  VIDEO_COMPRESSOR_CFG  *  pVideoCompressorParam,  LPCTSTR  tHint,  BOOL  bCapDev,  int  iIndex_sharedObj  )
{
	...
	VIDEO_COMPRESSOR_CFG  *  pVideoCompressor  =  &pVc->compressVideo.compressor;
...
int  BufferCB_v_rtsp(  MC_VAR_common  *  pProcInfo,  void  *  pGBUF_rtspCliHelp,  int  iIndex_capProcInfo,  int  iMediumType,  int  iDataType,  int  iCodecType,  int  iSampleTimeInMs,  unsigned  int  uiPts,  unsigned  char  *  pBuffer,  long  BufferLen,  bool  bSpsPps,  int  index_spsPps,  bool  bFinished_spsPps,  CB_rtsp_res  *  pRes  )
{
	...
	if  (  !pVc->bCaptureStarted  )  {
		...
		//
		BOOL  bPkt_sharedTex  =  FALSE;		//  2015/10/07
		//
...
int  BufferCB_a_rtsp(  MC_VAR_common  *  pProcInfo,  void  *  pGBUF_rtspCliHelp,  int  iIndex_capProcInfo,  int  iMediumType,  int  iDataType,  int  iCodecType,  int  iSampleTimeInMs,  unsigned  int  uiPts,  unsigned  char  *  pBuffer,  long  BufferLen,  void  *  pMsgTaskBufParam,  CB_rtsp_res  *  pRes  )
{
	...
	QY_TRANSFORM  *  pTransform  =  &pTransInfo->myTask.pTransforms[0];
		...
		//
		if  (  q2PostMsgAndTrigger(  pMsg,  len,  &pTransform->audio.q2  )  )  goto  errLabel;
...
int  BufferCB_av_rtsp_test_mp2t(  MC_VAR_common  *  pProcInfo,  void  *  pGBUF_rtspCliHelp,  int  iIndex_capProcInfo,  int  iMediumType,  int  iCodecType,  int  iSampleTimeInMs,  unsigned  char  *  pBuffer,  long  BufferLen,  bool  bSpsPps,  int  index_spsPps,  bool  bFinished_spsPps,  CB_rtsp_res  *  pRes  )
{
	HRESULT						hr					=	-1;
	//  
	QY_SHARED_OBJ			*	pSharedObj			=	NULL;
...
errLabel:
 ...
 BOOL  bGetCapturePara_rtsp(  void  *  pProcInfoParam,  int  iIndex_capBmp,  PF_BufferCB_av  pf,  unsigned  int  uiBufSize_pMsgBuf,  BOOL  bWait,  void  *  pVideoCapDevParam,  QY_VIDEO_HEADER  *  pVh_org  )
 {
	...
	//  2015/01/28
	 ...
	 //
	 DWORD  dwTickCnt_last  =  GetTickCount(  );
	 //  等待6秒吧
	 for  (  i  =  0;  i  <  60;  i  ++  )  {
		 if  (    pFrom->video.bGot_vh_stream  )  break;
		 //  2014/09/11
	 ...
	 DWORD  dwIntervalInMs_get_wh  =  GetTickCount(  )  -  dwTickCnt_last;
	 if  (  !bGot  )  {
