...
int testPcm2Aac_main(int argc, char *argv[])
{
	...
	const char *inFilename = "c:\\tttbbb\\124\\resample.pcm";
	const char *outFilename = "c:\\tttbbb\\124\\output.aac";
...
//	pCodec = avcodec_find_encoder_by_name("libfdk_aac");
	...
	swr_alloc_set_opts(swrCtx, av_get_default_channel_layout(pCodecCtx->channels), pCodecCtx->sample_fmt,
			pCodecCtx->sample_rate, av_get_default_channel_layout(pFrame->channels), src_sample_fmt, //PCM源文件的采样格式
			44100, 0, NULL);
	swr_init(swrCtx);
	...
	{
		//输入一帧数据的长度
		int length = pFrame->nb_samples * av_get_bytes_per_sample(src_sample_fmt) * pFrame->channels;
