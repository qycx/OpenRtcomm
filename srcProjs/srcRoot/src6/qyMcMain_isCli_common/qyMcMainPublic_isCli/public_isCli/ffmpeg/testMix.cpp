...
#if 10
{
    ...
    file = fopen("c:\\tttbbb\\124\\testMix.pcm", "ab+");
    if (NULL == file){
...
int testMix_main(int argc, char **argv)
{
    ...
    */
	char * fileName_in1=(char*)"c:\\tttbbb\\124\\filter_in1.wav";
	char * fileName_in2=(char*)"c:\\tttbbb\\124\\filter_in2.wav";
    ...
    if ((ret = open_input_file_1(fileName_in1)) < 0)
    {
        av_log(NULL, AV_LOG_ERROR, "open input file fail, ret: %d\n", ret);
        goto end;
    }
    if ((ret = open_input_file_2(fileName_in2)) < 0)
    {
        av_log(NULL, AV_LOG_ERROR, "open input file fail, ret: %d\n", ret);
        goto end;
    }
