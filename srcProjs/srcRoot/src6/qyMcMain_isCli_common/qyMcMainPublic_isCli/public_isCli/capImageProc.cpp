						...
						int  *  piX_dst,  int  *  piY_dst,  unsigned  int  *  piW_dst,  unsigned  int  *  piH_dst  )
{
 ...
 //
#if 0
__declspec(  dllexport  )  int  noA_new_getCapImagesFunc(  unsigned  char  ucbAutoClip,  unsigned  char  ucb3d,  int  iX_desc,  int  iY_desc,  int  iW_desc,  int  iH_desc,  PARAM_getCapImages  *  pParam,  CAP_images_head  *  pImgsHead,  CAP_IMAGE  *  pImgMems,  int  uiSize_pImgMem,  unsigned  int  uiMaxCnt_imgMems  )
{
	 int					iErr	=	-1;
	 int					i;
	 CAP_IMAGE			*	pImg;
...
{
	if (!pImg->iTaskId)  return false;
	if (!pImg->tn_a && !pImg->uiTranNo_openAvDev)  return  false;
	if (pImg->uiTranNo_openAvDev) {
		if (!pImg->iW_org || !pImg->iH_org)  return false;
	}
	//
	return  true;
}
...
#if 0
		//if (pImg->iW_org && pImg->iH_org
		if (!pImg->tn_a && !pImg->uiTranNo_openAvDev)  continue;
		if (!pImg->iTaskId)    continue;
#endif
		//
		if (!bCapImgOk(pImg))  continue;
...
#endif
		//
		if  (  ucbFixedSize  )  myGetImageInfo_2d(  TRUE,  ucbAutoClip,  pImg->iW_org,  pImg->iH_org,  iX,  iY,  iW,  iH,  (  unsigned  int  *  )&pImg->iW_i,  (  unsigned  int  *  )&pImg->iH_i,  &pImg->iX_src_i,  &pImg->iY_src_i,  &pImg->iX_dst,  &pImg->iY_dst,  (  unsigned  int  *  )&pImg->iW_dst,  (  unsigned  int  *  )&pImg->iH_dst  );
		else  {
			  //
...
errLabel:
	...
	if  (  bModified_capImg  )  {
		...
		timestamp_renew(  NULL,  &pImgs->head.ts_images,_T("drawCapImages.l5286"));  //  2017/10/17
	}
  ...
  int  old_freeCapImageByTaskId(  HWND    hDlg,  int  idc,  int  iTaskId,  CAP_IMAGES  *  pImgs,  HWND  hWnd_dst  )
 {
	 ...
	 for  (  i  =  0;  i  <  mycountof(  pImgs->mems  );  i  ++  )  {
		  ...
		  if  (  pImg->iTaskId  ==  iTaskId  )  {
			  ...
			  //  2016/02/21
			  //pImgs->head.dwModifiedTickCnt_images  =  GetTickCount(  );
			  timestamp_renew(  NULL,  &pImgs->head.ts_images,_T("old_freeCapImageByTaskId.l5458"));
		  }
	 }
  ...
  int  freeCapImageByTaskId(HWND    hDlg, HWND  hCtrl, int  iTaskId, CAP_IMAGES* pImgs, HWND  hWnd_dst)
  {
	  ...
	  for (i = 0; i < mycountof(pImgs->mems); i++) {
		  ...
		  if (pImg->iTaskId == iTaskId) {
			  ...
			  //  2016/02/21
			  //pImgs->head.dwModifiedTickCnt_images  =  GetTickCount(  );
			  timestamp_renew(NULL, &pImgs->head.ts_images,_T("freeCapImageByTaskId.l5490"));
		  }
	  }
 ...
 int  old_freeCapImageBySth(  CTX_qm_thread  *  pCqt,  HWND    hDlg,  int  idc,  int  iTaskId,  QY_MESSENGER_ID	*	pIdInfo,  CAP_IMAGES  *  pImgs,  HWND  hWnd_dst  )
{
	 ...
	 for  (  i  =  0;  i  <  mycountof(  pImgs->mems  );  i  ++  )  {
		  ...
		  timestamp_renew(  pCqt,  &pImgs->head.ts_images,_T("old_freeCapImageBySth.l5530"));
	 }
 ...
 int  freeCapImageBySth(CTX_qm_thread* pCqt, HWND    hDlg, HWND  hCtrl, int  iTaskId, QY_MESSENGER_ID* pIdInfo, CAP_IMAGES* pImgs, HWND  hWnd_dst)
 {
	 ...
	 for (i = 0; i < mycountof(pImgs->mems); i++) {
		 ...
		 //
		 //pImgs->head.dwModifiedTickCnt_images  =  myGetTickCount(  pCqt  );  //  2017/09/28
		 timestamp_renew(pCqt, &pImgs->head.ts_images,_T("freeCapImageBySth.l5565"));
	 }
 ...
 int  old_freeCapImages(  HWND    hDlg,  int  idc,  CAP_IMAGES  *  pImgs,  HWND  hWnd_dst,  HDC  *  phDc_dst,  LPCTSTR  hint  )
{
	 ...
	 timestamp_renew(  NULL,  &pImgs->head.ts_images,_T("old_freeCapImages.l5599"));	//  2017/10/17
	 //
 ...
 int  freeCapImages(HWND    hDlg, HWND  hCtrl, CAP_IMAGES* pImgs, HWND  hWnd_dst, HDC* phDc_dst)
 {
	 ...
	 //
	 //pImgs->head.dwModifiedTickCnt_images  =  0;	//  2017/09/28
	 timestamp_renew(NULL, &pImgs->head.ts_images,_T("freeCapImages.l5634"));	//  2017/10/17
	 //
	 if (hWnd_dst && phDc_dst && *phDc_dst) {
		 int		iRet;
