...
errLabel:
 ...
 //
#if  0
 int  iitryToFreePlayer(DLG_TALK_videoConferenceActiveMemFrom  *  pAct,  LPCTSTR  hint )
 {
	 QY_PLAYER* pPlayer;
	 int  i;
	 ...
	 for (i = 0; i < pProcInfo->av.usCnt_players; i++) {
		...
		//
		 ...
		 //
		 if ( pAct->avStream.idInfo.ui64Id == pPlayer->idInfo_recorder.ui64Id
			 && pAct->avStream.obj.tranInfo.audio.uiTranNo_openAvDev == pPlayer->audio.uiTranNo
			 && pAct->avStream.obj.tranInfo.video.uiTranNo_openAvDev == pPlayer->video.uiTranNo)
		 {
			 //
			 _sntprintf(tBuf, mycountof(tBuf), _T("tryToFreePlayer: index_player %d. %s"), i,  hint );
	...
	if  (  bRequestToSpeak  )  {
		...
		//
		  ...
		  if  (  i  <  pVc->usCntLimit_activeMems_from  )  {
			  //
			  if (1) {
				  //tryToFreePlayer(&pVc->activeMems_from[i],  _T("confStarter_requestToSpeak. l606"));
			  }			  
			  //
