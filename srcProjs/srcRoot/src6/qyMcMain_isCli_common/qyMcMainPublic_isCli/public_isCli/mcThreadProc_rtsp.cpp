...
int  tmp_getResp(  MC_VAR_common  *  pProcInfo,  VIDEO_capDev_rtsp  *  pRtsp,  RTSP_usrData_resp  *  pResp  )
{
	...
	DVT_decV  *  pDvt_decV  =  pRtsp->pDvt_decV;
...
{ 
	...
	while  (   !bThreadQuit_videoCapDevRtsp(  pRtsp  )  )  { 
		   ...
		   //
		   BufferCB_v_rtsp(  pProcInfo,  NULL,  iIndex_capBmp,  pMsg->iMediumType,  iDataType,  pMsg->iCodecType,  pMsg->iSampleTimeInMs,  pMsg->uiPts,  (  unsigned  char  *  )pMsg->buf,  pMsg->uiLen,  pMsg->bSpsPps,  pMsg->index_spsPps,  pMsg->bFinished_spsPps,  &res  );    
...
extern "C" DWORD WINAPI mcThreadProc_recvRtsp( LPVOID lpParameter )
{
	...
	//
	int  iIndex_capBmp  =  0;
	set_PF_BufferCB_rtsp(  BufferCB_av_rtsp_test_mp2t,  NULL,  iIndex_capBmp,  0  );
