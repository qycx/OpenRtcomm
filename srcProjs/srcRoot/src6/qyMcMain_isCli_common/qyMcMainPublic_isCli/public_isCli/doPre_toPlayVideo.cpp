...
//
extern "C" DWORD test_doPre_toPlayVideo(LPVOID lpParameter);
//
extern "C" __declspec(dllexport) DWORD doPre_toPlayVideo(LPVOID pPlayerParam);


//
#if  1
//
#define  __TEST_doPre_toPlayVideo__
//
#endif 
...
//
#define		CONST_cfgName_aiResize				_T(  "aiResize")
bool  bSupported_aiResize()
{
	...
	//
	TCHAR  cfgVal[128];  cfgVal[0] = 0;

	//USB Video Device
	getCfgValByNameT(pQyMc->cfg.smCfgFile, (TCHAR*)CONST_cfgName_aiResize, cfgVal, mycountof(cfgVal));
	tTrim(cfgVal);
...
extern "C" DWORD WINAPI mcThreadProc_doPre_toPlayVideo(LPVOID lpParameter)
{
	...
	//
	bool  bEnable_aiResize;  bEnable_aiResize = false;
	bEnable_aiResize = bSupported_aiResize();


	//
	if (pFuncs->aiResize.pf_doPre_toPlayVideo) {
		pFuncs->aiResize.pf_doPre_toPlayVideo(lpParameter,bEnable_aiResize);
	}
	else {

#ifdef  __TEST_doPre_toPlayVideo__

		test_doPre_toPlayVideo(lpParameter);

#else
		//
		doPre_toPlayVideo(lpParameter);
#endif 
...
extern "C" DWORD test_doPre_toPlayVideo(LPVOID lpParameter)
{
	...
	QY_PLAYER* pPlayer = (QY_PLAYER*)lpParameter;
	QY_MC* pQyMc = g_pQyMc;
	if (!pQyMc)  return  -1;
