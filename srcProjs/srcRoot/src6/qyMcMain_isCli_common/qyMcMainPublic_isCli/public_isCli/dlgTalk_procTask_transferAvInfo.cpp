...
int  getAvLevel_reply(  TRANSFER_AV_INFO  *  pReq  )
{
	...
	else  avLevel  =  CONST_policyAvLevel_240p;
	 ...
	 )
 {
	 ...
	 switch  (  usOp  )  {
				...
				case  CONST_imOp_dual_permit:  { 
					  ...
					  int level  =  getAvLevel_reply(  &pContent->transferAvInfo  );
					  ...
					  if  (  myGetAvCompressorCfg(  CONST_capType_av,  0,  0,  level,  &avCompressor_reply  )  )  goto  errLabel;	//  2014/08/15
						  ...
						  )
					  {
						  ...
						  if  (  pContent->transferAvInfo.confCfg.ucbVideoConference  )  {
							  //
#if 0
							  if  (  pContent->transferAvInfo.confCfg.conf.conf_iFourcc  )  {						  
								  iFourcc2Str(  pContent->transferAvInfo.confCfg.conf.conf_iFourcc,  avCompressor_reply.video.common.fourccStr,  mycountof(  avCompressor_reply.video.common.fourccStr  )  );						  
								  switch  (  pContent->transferAvInfo.confCfg.conf.conf_iFourcc  )  {
										  case  CONST_fourcc_vp80:
											    avCompressor_reply.video.common.ucCompressors  =  CONST_videoCompressors_vp8;
											    break;
	  case  CONST_fourcc_h264:
											    avCompressor_reply.video.common.ucCompressors=CONST_videoCompressors_hwAccl;
											break;
						  ...
						  //
#if  0
						  unsigned  short  usMaxFps_toShareBmp_real  =  avCompressor_reply.video.common.usMaxFps_toShareBmp;
						  if  (  pContent->transferAvInfo.confCfg.conf.usMaxFps_toShareBmp_conf  )  {
							  usMaxFps_toShareBmp_real  =  min(  usMaxFps_toShareBmp_real,  pContent->transferAvInfo.confCfg.conf.usMaxFps_toShareBmp_conf  );
							  //
							  if  (  avCompressor_reply.video.common.iAvgBitrate  )  {
								  if  (  usMaxFps_toShareBmp_real  <  avCompressor_reply.video.common.usMaxFps_toShareBmp  )  {
#if 0
									  if  (  avCompressor_reply.video.common.usMaxFps_toShareBmp  )  {
										  avCompressor_reply.video.common.iAvgBitrate  =  avCompressor_reply.video.common.iAvgBitrate  *  usMaxFps_toShareBmp_real  /  avCompressor_reply.video.common.usMaxFps_toShareBmp;
									  }
#endif
									  //
									  avCompressor_reply.video.common.usMaxFps_toShareBmp = usMaxFps_toShareBmp_real;
									  avCompressor_reply.video.common.iAvgBitrate = 0;
									  avCompressor_reply.video.common.iAvgBitrate = myGetBitrate(&avCompressor_reply.video.common, avCompressor_reply.video.common.iWidth_pic, avCompressor_reply.video.common.iHeight_pic);
								  }
							  }
						  }
#endif
						  ...
						  iFourcc2Str(pContent->transferAvInfo.ass.tranInfo.video.vh_stream.dwFourcc, avCompressor_reply.video.common.fourccStr, mycountof(avCompressor_reply.video.common.fourccStr));
						  switch (pContent->transferAvInfo.ass.tranInfo.video.vh_stream.dwFourcc) {
						  case  CONST_fourcc_vp80:
							  avCompressor_reply.video.common.ucCompressors = CONST_videoCompressors_vp8;
							  break;
						  case  CONST_fourcc_h264:
					  ...
					  */
					  ...
					  if  (  rcd.uiType  !=  CONST_imTaskType_transferAvInfo  )  {
						  ...
						  if  (  m_var.addr.uiObjType  ==  CONST_objType_imGrp  )  {									
							  if  (  pMsgTask->data.route.idInfo_to.ui64Id  ==  m_var.pMisCnt->idInfo.ui64Id
								  &&  pContent->transferAvInfo.idInfo_logicalPeer.ui64Id  ==  m_var.addr.idInfo.ui64Id  )
							  {	
								  if  (  m_var.av.taskInfo.bTaskExists  									
									  &&  m_var.av.taskInfo.ucbStarter  )																			
								  {
									  if  (  m_var.av.taskInfo.mosaicMaker.video.resObj.uiObjType								
										  ||  m_var.av.taskInfo.mosaicMaker.resource.resObj.uiObjType  )  											
									  {
										  bDo_avProxy  =  TRUE;									
									  }								  
	  								  //
								  ...
								  //
										  ...
										  #if  1
												bDo_avProxy  =  TRUE;
												traceLog((TCHAR*)  _T(  "For test: bExists_avProxy"  )  );
										  #endif
										  }
								  #endif
								  ...
								  if  (  !bDo_avProxy  )  {
									  TCHAR  tBuf[128];
									  _sntprintf(  tBuf,  mycountof(  tBuf  ),  _T(  "Application for avProxy from %I64u is denied. No mosaic exists"  ),  pMsgTask->data.route.idInfo_from.ui64Id  );
									  showInfo_open0(  0,  0,  tBuf  );
						  ...
						  if  (  bDo_avProxy  )  {
							  #ifdef  __DEBUG__
									  traceLog((TCHAR*)  _T(  "bExists_avProxy is true"  )  );
							  #endif

							  //
							  /*
							  CQyMalloc		mallocObj_transferAvInfo;
							  ...
							  */
							  ...
							  TRANSFER_AV_INFO  transferAvInfo_req;//=pTaskInfo->var.;
						  						
							  memset(  &transferAvInfo_req,  0,  sizeof(  transferAvInfo_req  )  );
							  transferAvInfo_req.uiType  =  CONST_imCommType_transferAvInfo;
							  //  2016/04/23
							  transferAvInfo_req.tmp_usSubtype  =  CONST_transferAvInfo_subtype_task;

							  //
							  transferAvInfo_req.uiTaskType  =  pContent->transferAvInfo.uiTaskType;
								
							  //
							  transferAvInfo_req.ass.tranInfo.video.uiTranNo_openAvDev  =  getuiNextTranNo(  0,  0,  0  );	
							  transferAvInfo_req.ass.tranInfo.audio.uiTranNo_openAvDev  =  transferAvInfo_req.ass.tranInfo.video.uiTranNo_openAvDev;
							  
							  //  
							  transferAvInfo_req.proxiedTranInfo.idInfo.ui64Id  =  pMsgTask->data.route.idInfo_from.ui64Id;
							  transferAvInfo_req.proxiedTranInfo.uiObjType  =  CONST_objType_proxiedStream_video;
							  //
							  transferAvInfo_req.proxiedTranInfo.ti  =  pContent->transferAvInfo.ass.tranInfo;
								
							  //
							  PROC_TASK_AV  *  pTask  =  &task.myTask;//&transferAvInfo.myTask;	//  get_transferAvInfo_pTask(  &transferAvInfo,  _T(  ""  )  );
							  ...
							  //
							  //  2014/09/23							
							  pTask->mem0_from.shClientVer  =  pContent->transferAvInfo.ass.tranInfo.head.shClientVer;
							  //
							  //  2014/11/11							
							  memcpy(  &pTask->mem0_from.video.tv_recvd,  &pContent->transferAvInfo.ass.tranInfo.video,  sizeof(  pTask->mem0_from.video.tv_recvd  )  );
							  //
							  //  2014/11/11							
							  memcpy(  &pTask->mem0_from.audio.ta_recvd,  &pContent->transferAvInfo.ass.tranInfo.audio,  sizeof(  pTask->mem0_from.audio.ta_recvd  )  );
							  ...
							  if  (  !pTask->ucbVideoConference  )  {
								  ...
								  tmpGetMaxAudioPlayCfg(  &pContent->transferAvInfo.ass.tranInfo.audio.compressor.playCfg,  &avCompressor_reply.audio.playCfg,  &pTask->audio.playCfg  );								
								  }								
							  else  {	//  2009/05/31
							  ...
							  //
							  lenInBytes  =  sizeof(  transferAvInfo_req  );
							  ...
							  QY_MESSENGER_ID  idInfo_dst;
							  idInfo_taskSender.ui64Id  =  m_var.pMisCnt->idInfo.ui64Id;
							  ...
							  uiChannelType  =  0;
							  //
							  if  (  postMsgTask2Mgr_mc(  m_var.pMisCnt,  CONST_misMsgType_task,  0,  CONST_qyCmd_sendMedia,  tStartTran,  uiTranNo,  0,  pMsgTask->iTaskId,  pMsgTask->uiTaskType,  (  char  *  )&transferAvInfo_req,  lenInBytes,  &m_var.addr.idInfo,  &idInfo_taskSender,  &idInfo_taskReceiver,  &idInfo_dst,  uiChannelType,  NULL,  FALSE  )  )  goto  errLabel;
					  ...
					  if  (  rcd.uiContentType  ==  CONST_imCommType_transferAvInfo  )  {
									...
									||  rcd.uiType  ==  CONST_imTaskType_shareDevice_grp  )  
								{
									...
									//
									pProcInfo->dlgTalk_confStart_post(hDlgTalk, _T("dlgTalk_procTask_av: l1598"));
									...
									||  rcd.uiType  ==  CONST_imTaskType_shareDevice_grp)  
								{
									...
									//
								#if  0
									if  (  pContent->transferAvInfo.confCfg.ucbAvConsole
										||  pQyMc->iCustomId  ==  CONST_qyCustomId_hzj		//  2018/10/24
										)  
									{
										//
										if  (  ucbSendLocalAv  &&  
