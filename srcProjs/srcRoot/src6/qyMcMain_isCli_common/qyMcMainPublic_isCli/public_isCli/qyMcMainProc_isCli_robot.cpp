...
#endif
	 if (GetCurrentThreadId() != pQyMc->gui.ctx_gui_thread.dwThreadId9) {
...
int  initTaskAvFrom_audio(  MC_VAR_common  *  pProcInfo,  DLG_TALK_videoConferenceActiveMemFrom  *  pActiveMem_from,  int  index_activeMems_from,  QY_TRANSFORM  *  pTransform  )
{
	...
	#ifndef  __WINCE__
			hadid  =  find_driver(  pActiveMem_from->avStream.obj.tranInfo.audio.ah_compress.wfx.wFormatTag,  pActiveMem_from->avStream.obj.tranInfo.audio.ah_compress.wfx.nChannels,  pActiveMem_from->avStream.obj.tranInfo.audio.ah_compress.wfx.nSamplesPerSec,  pActiveMem_from->avStream.obj.tranInfo.audio.ah_compress.wfx.wBitsPerSample  );
			if  (  hadid  ==  NULL  )  {
				#ifdef  __DEBUG__
						traceLogA(  (char*)  "initTaskAvFrom_audio failed. No driver found."  );
				#endif
