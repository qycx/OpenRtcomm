 ...
 int  toShareDynBmp(  CCtxQmc  *  pProcInfo,  HWND  m_hWnd,  void  **  ppCapStuff,  int  iTaskId_suggested,  AV_COMPRESSOR_CFG  *  pCompressor_suggested,  int  iFourcc_suggested,  TASK_av_props  *  pTaskAvProps,  SHARE_dyn_bmp  *  pDynBmpMem  )
{
	...
	//  		
	//  2014/04/06		
	unsigned  int  uiCamCapType  =  CONST_camCapType_directX;
	switch  (  uiObjType  )  {
			case  CONST_objType_rtspStream:  
	...
	//  2015/10/21
	BOOL  bSupported_pktResType_sharedTex  =  FALSE;
...
{	
	...
	switch  (  uiObjType  )  {
			...
			default:				{
					...
					if  (  procSendAvInfo(  pMisCnt,  m_hWnd,  FALSE,  uiTaskType,  &tmpParam, pCompressVideo,  pCompressAudio,  iIndex_sharedObj,  iIndex_sharedObjUsr,  &rule_dynBmp,  NULL,  &tmp_iTaskId,  NULL,  NULL  )  )  goto  errLabel;
