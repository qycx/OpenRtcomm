...
UINT WM_avRecord_cursor;
UINT WM_avRecord_start;
UINT WM_avRecord_stop;
					 ...
					 )
{
	switch  (  Reason  )  { /* reason */
			case  DLL_PROCESS_ATTACH:
				  ...
				  WM_avRecord_cursor			= ::RegisterWindowMessage(  CONST_wmStr_avRecord_cursor  );
				  WM_avRecord_start			= ::RegisterWindowMessage(  CONST_wmStr_avRecord_start  );
				  WM_avRecord_stop			= ::RegisterWindowMessage(  CONST_wmStr_avRecord_stop  );
	...
	{ /* pass it on */
	 ...
	 if(msg->message == WM_MOUSEMOVE || msg->message == WM_NCMOUSEMOVE || msg->message == WM_LBUTTONDOWN || (msg->message == WM_LBUTTONUP)) {
		 ...
		 if (difftime>20) {	//up to 50 frames per second	 
			...
			PostMessage(hWndServer, WM_avRecord_cursor , (unsigned int) hcur, msg->message);
