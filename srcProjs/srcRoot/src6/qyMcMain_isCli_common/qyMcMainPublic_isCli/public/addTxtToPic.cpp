...
int  updateImg_addTxt(  QY_MC  *  pQyMc,  LPCTSTR  videoHint,  BITMAPINFO  *  pBmi_pic,  BOOL  bLine0,  	SUBTITLE_cfgEx  *  pMemCfg,  ADD_txt_mem  *  pMem,  BOOL  *  pbModified_img,  LPCTSTR  dbgHint  )
{
	...
	BOOL  bDemo  =  (  pShmCmds  &&  pShmCmds->ucbDaemonDemo  );
	...
	if  (  !hint[0]  )  {  //  这里表明没有txt
		...
		//
		//  2013/03/11
		G_guiData_qyMc  *  pGuiData  =  (  G_guiData_qyMc  *  )pProcInfo->get_g_guiData(  );  //  pFuncs->pf_get_g_guiData(  );
		if  (  !pGuiData  )  goto  errLabel;
		

		HFONT				hFont_toTextOut		=	NULL;
		int  iW_txtImg  =  0;	//  pBmi_pic->bmiHeader.biWidth;
		int  iH_txtImg  =  0;
		...
		//  
		int	nCount  =  GetDIBits(  hDc_mem,  hBmp,  0,  pBmi_txt->bmiHeader.biHeight,  pTxtBuf,  (  BITMAPINFO  *  )pBmi_txt,  DIB_RGB_COLORS  );
