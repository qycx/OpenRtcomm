...
__declspec(  dllexport  )  int dlgMcClientLogon_OnInitDialog(  HWND  hDlg,  void  *  pVarParam  )
{
	...
	QY_MC			*	pQyMc					=	m_var.guiData.pQyMc;
	if  (  !pQyMc  )  return  -1;
	CCtxQmc_gui* pProcInfo = (CCtxQmc_gui*)QY_GET_procInfo_isCli();
	...
	//  2011/03/23
	int nElapseInMs = 1000;
	nElapseInMs = 100;
	m_var.uiTimerId  =  SetTimer(  hDlg,  1,  nElapseInMs,  NULL  );
	...
	if (pProcInfo->m_bUseKeyToLogin_forQmcGui  ) {
		...
		//
		m_var.cntCfg[m_var.iSeqNoSelected].ucbAutoLogon = true;
	}
...
errLabel:
 ...
 void dlgMcClientLogon_OnTimer(HWND  hDlg, UINT_PTR nIDEvent)
 {
	 ...
	 DLG_mcClientLogon_var& m_var = pDlg->m_var;
	 QY_MC* pQyMc = m_var.guiData.pQyMc;
	 CCtxQmc_gui* pProcInfo = (CCtxQmc_gui*)pQyMc->get_pProcInfo();
	 ...
	 //
#ifdef  __DEBUG__
		//traceLog(_T("dlgMcClientLogon_OnTimer called"));
#endif
	...
	//
	 if (pProcInfo->m_bUseKeyToLogin_forQmcGui) {
		 ...
		 if (!m_var.bDone_devLogin) {
			 Var_ca_dev_qmc* pVc = &pProcInfo->m_var.ctxSm.ca_dev;

			 _sntprintf(tBuf, mycountof(tBuf), _T("%d: %s"), pQyMc->appParams.iSeqNoSelected_appObjPrefix, pVc->tStatusBuf);
			 //
			 SetWindowText(hDlg, tBuf);


			 bool  bNeedAccept = false;
			 smLogin_onTimer((HWND)hDlg, &m_var, &pProcInfo->m_var.ctxSm, &bNeedAccept);
			 if (bNeedAccept) {
				 //this->accept();
				 //EndDialog(hDlg, IDOK);
				 //return;
				 m_var.bDone_devLogin = true;
			 ...
			 if (!m_var.m_bStartToUsrLogin) {
				 ...
				 if (!pVc->bExists_usrKey) {
					 safeTcsnCpy(pProcInfo->m_var.ctxSm.smTerminalInitCfg.fake_usrName, pVc->usrLogin.loginUsingName.tUsrName, mycountof(pVc->usrLogin.loginUsingName.tUsrName));
					 safeStrnCpy(pProcInfo->m_var.ctxSm.smTerminalInitCfg.fake_passwd, pVc->usrLogin.loginUsingName.passwd, mycountof(pVc->usrLogin.loginUsingName.passwd));
				 }
			 ...
			 smUsrLogin_onTimer(hDlg, &m_var, &pProcInfo->m_var.ctxSm, bNeedShowLogon, bNeedClose,pProcInfo->uiTerminalType);
			 if (bNeedClose) {
				 EndDialog(hDlg, IDOK);
				 //
...
__declspec(  dllexport  )  int  dlgMcClientLogon_OnOK(  HWND  hDlg,  LPTSTR  m_name,  LPTSTR  m_passwd  )
{
	...
	//
	if (pProcInfo->m_bUseKeyToLogin_forQmcGui) {
		showInfo_open0(0, 0, _T("for useKeyToLogin, cant click Ok"));
		return  -1;
	}
	...
	for  (  i  =  0;  i  <  pProcInfo->ucCnt_misCnts;  i  ++  )  
	{
		 ...
		 if  (  pChannel->bSessionOpen  )  {
			 qnmCloseSession_msg(  mynull,&pChannel->session,  &pChannel->sock,  &pQyMc->cfg.to,  _T("dlgMcClientLogon_OnOK.l731"));
		 ...
		 if  (  !pChannel->bSessionOpen  )  {
			 ...
			 m_var.respHint[0]  =  0;
			 if (!doMisCntLogon(pMisCnt, pChannel, pDlg->m_hWnd, m_var.respHint, mycountof(m_var.respHint))) {
	...
	//
	if (pProcInfo->do_afterLogonOk()) {
		showInfo_open0(0, 0, _T("do_afterLogonOk failed"));
		goto  errLabel;
	}
