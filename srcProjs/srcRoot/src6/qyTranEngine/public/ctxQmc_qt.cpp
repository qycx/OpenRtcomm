...
bool CCtxQmc_qt::myDestroyWindow(HWND  hWnd)
{
    ...
    QWidget* pWnd = (QWidget*)wmBuf.u.getObjAddr.pObjAddr;
...
int  CCtxQmc_qt::displayRecentFriends(MIS_MSG_displayRecentFriends_qmc* pMsg)
{
    ...
    HWND hTalk_vide = m_var.hTalk_video;

    CDlgTalk_qt* pTalk = (CDlgTalk_qt*)QWidget::find((WId)hTalk_vide);
...
int  CCtxQmc_qt::confKeyChanged(HWND  hDlgTalk)
{
    ...
    //
    CDlgTalk_qt* pTalk = (CDlgTalk_qt*)CDlgTalk_qt::find((WId)hDlgTalk_mgr);
    if (pTalk) {
...
//
int CCtxQmc_qt::popup_permitToSpeak(HWND  hDlgTalk)
{
...
int  CCtxQmc_qt::do_logImMsg_isCli(void* pDb, int  iDbType, void* pDBManager, IM_MSG_RCD* pRcd)
{
    ...
    if (pRcd->uiType == CONST_imCommType_htmlContent) {
        ...
        if (pRcd->idInfo_initiator.ui64Id == 0) {
            ...
            getTalkerDesc(pRcd->idInfo_initiator, &uiObjType_from, grpName_from, mycountof(grpName_from), null, null, 0, null, 0, srcName_from, mycountof(srcName_from));
        }
        
    }

    //getTalkerDesc(pRcd->idInfo_send, &uiObjType_from, grpName_from, mycountof(grpName_from), null, null, 0, null, 0, srcName_from, mycountof(srcName_from));
    getTalkerDesc(pRcd->idInfo_recv, &uiObjType_peer, grpName_peer, mycountof(grpName_peer), null, null, 0, null, 0, srcName_peer, mycountof(srcName_peer));
    ...
    getTalkerDesc(pRcd->idInfo_logicalPeer, &puiObjType, null, 0, null, null, 0, null, 0, null, 0);
    if (puiObjType == CONST_objType_imGrp) {
