...
namespace {
    ...
    const static int DEFAULT_VIDEO_WIDTH = 484;// 450;
    const static int DEFAULT_VIDEO_HEIGHT = 800;

    const static int DEFAULT_VIDEO_WIDTH_768 = 450;// 450;
...
void CDlgTalk_qt::autoSet(unsigned  int uiTranNo_shadow) {

    if (!uiTranNo_shadow) {

    }
    else {
        //QString titleName = ui->titleName->text();
        ui->btnDisk->click();
    }
...
void CDlgTalk_qt::clearScrollBar()
{
    HWND  hDlgTalk = (HWND)this->winId();
    DLG_TALK_var* pm_var = get_pm_var();
    if (pm_var == null)return;
    DLG_TALK_var& m_var = *pm_var;

    if (pm_var->av.taskInfo.bTaskExists) {

        //
        freeCapImages(hDlgTalk, m_var.av.hCtrl_peer, &m_var.av.peerZone.images, null, &m_var.av.hDc_peer);
...
int CDlgTalk_qt::doTask_av(int iCmd, int iTaskId)
{
    ...
    case  IDC_av: {
        ...
        //
        doCmd_startAvCall(null, hCurTalk, level, b3D, false, &param);
    }
                break;
    default:
        break;
...
int  CDlgTalk_qt::do_closeTaskAv_afterTaskClosed()
{
    ...
    showInfo_open0(0, null, _T("do_closeTaskAv_afterTaskClosed: close talk_video shadow now"));
...
int  talk_doAv(  HWND  hParent,  QY_MESSENGER_ID idInfo)
{
    ...
    //if (isTalkerShadowMgr(pm_var->addr)) return;
    unsigned  int  uiObjType = 0;
    if (getTalkerDesc(idInfo, &uiObjType, null, 0, null, null, 0, null, 0, null, 0))  return  -1;
    ...
    if (bExists_activeTaskAv(pQyMc,null, &idInfo_taskAv)) {
    ...
    //
    pProcInfo->tryToTalkToMessenger_any(null, idInfo.ui64Id, iTalkSubtype, FALSE, FALSE, &hWnd);
...
void CDlgTalk_qt::procSendFile_qt(QString strFileName)
{
    ...
    //
    TCHAR* finalName = _tcsrchr(fileName, _T('/'));
    if (finalName == null)return;
    finalName++;


    //
    procSendFile(fileName, finalName, null, hMgr, pMgrVar, &iTaskId);
...
int  CDlgTalk_qt::do_afterInit()
{
    ...
    if (isTalkerShadowMgr(m_var.addr)) {
        pQyMc->dbg.dwTickCnt_dlgTalk_0_inited = myGetTickCount(null);
        int  iElapseInMs = pQyMc->dbg.dwTickCnt_dlgTalk_0_inited - pQyMc->dwTickCnt_logon;
        int  ii = 0;
    }
    else {
        pQyMc->dbg.dwTickCnt_dlgTalk_1_inited = myGetTickCount(null);
...
void CDlgTalk_qt::addShowMsg(QString msg, int64_t idinfo, int msgTime, QString msgid, int msgtype, int  iTaskId, int fileStatus, int chatType, int isMore ,int filesize)
{
    ...
    // 获取当前时间
    if (msgTime == 0) {
        tRecvTime = timeCurrent.toTime_t(); ;
    }
    else {
...
void CDlgTalk_qt::showFileProgress(QString msgid, QString Progress, int iStatus , qint64 idinfo_to) {
    ...
    int isObjType = 0;
    //判断群组
    getTalkerDesc(userId_type, &uiObjType, null, null, null, null, null, null, null, null,null);
    if (  uiObjType == CONST_objType_imGrp)
    {
        isObjType = 1;
    }
    ...
    /*if (isTalkerShadowMgr(pCurVar->addr))  return;*/
    getTalkerDesc(idInfo_to, null, null, null, null, null, null, null, null, idinfoto_name, mycountof(idinfoto_name));
...
int  CDlgTalk_qt::refreshLayout()
{
    ...
    if (pMgrVar->addr.uiObjType != CONST_objType_messenger) {   //  imGrp, tmpMsgr(临时组)
        ...
        getTalkerDesc(pCurVar->addr.idInfo, NULL, grpName_peer, mycountof(grpName_peer), null, null, 0, null, 0, NULL, 0);
    ...
    //
        ...
        //
        getTalkerDesc(pCurVar->addr.idInfo, NULL, null, 0, null, null, 0, null, 0, srcName_peer, mycountof(srcName_peer));
...
//实时监控消息
bool CDlgTalk_qt::nativeEvent(const QByteArray& eventType, void* message, long* result)
{
    Q_UNUSED(eventType);
...
int  CDlgTalk_qt::mySizeAllControls_dlgTalk_peer(HWND  hDlg, DLG_TALK_var& m_var, DLG_talk_layout* pLayout, RECT* pRect)
{
    ...
    old_freeCapImages(hDlg, m_var.guiData.iIDC_STATIC_peerDesc, &m_var.av.peerZone.images, old_M_GetDlgItem(hDlg, idc_dst), &m_var.av.hDc_peer);		//  2013/02/28. 最好能根据me,other,peer的任何size变化来决定是否free
    ...
    if (pMgrVar->addr.uiObjType == CONST_objType_imGrp) {
        ...
        if (pMgrVar->av.taskInfo.mosaicTrans.status.ucb2Streams) {
            ...
            if (pActiveMem->avStream.obj.resObj.uiObjType == CONST_objType_mosaicStream_resource) {
                bEnableScrollBar(true, pActiveMem->avStream.obj.tranInfo.video.vh_decompress.bih.biWidth, pActiveMem->avStream.obj.tranInfo.video.vh_decompress.bih.biHeight);
            }
            else {
                bEnableScrollBar(false, 0, 0);
            }
...
void CDlgTalk_qt::on_SpeakBtn_click() {
    ...
    //  2010/09/02
    pFuncs->pf_setCurSharedObjUsr_localAv(pProcInfo, pDlgTalkVar->av.iIndex_sharedObj_localAv, hMgr);
...
void  CDlgTalk_qt::on_EndAvBtn_click() {
    ...
    //
    showInfo_open0(0, null, _T("endAv: close talk_video now"));
...
void CDlgTalk_qt::updateMemStatus(qint64 idInfo, unsigned  short status)
{
    ...
    for (int i = 0; i < row; i++)
    {
        ...
        if (idInfo_item->text() == QString::number(idInfo) || pMisCnt->idInfo.ui64Id == idInfo_item->text().toInt())
        {
            ...
            getTalkerDesc(idInfo_on, null, null, 0, null, dw, mycountof(dw), bm, mycountof(bm), syr, mycountof(syr));
            qstr = QString::fromUtf16((char16_t*)dw) + " " + QString::fromUtf16((char16_t*)bm) + " " + QString::fromUtf16((char16_t*)syr);
...
int  tmpHandler_printImGrpMemList_grpMembers_contr(void* p0, void* p1, void* p2)
{
    ...
    //  if  (  pQMem->uiType  ==  CONST_objType_imGrp  )
    {
        ...
        if (QString::number(pQMem->idInfo_grp.ui64Id) == pPeer_id) {
            ...
            getTalkerDesc(pQMem->idInfo_mem, NULL, grpName_mem, mycountof(grpName_mem), null, dw, mycountof(dw), bm, mycountof(bm), srcName_mem, mycountof(srcName_mem));
            MemberInfo data;
...
void CDlgTalk_qt::initConfMem(QString searchStr) {
   ...
   // ui->tableWidget->setColumnWidth(3, 5);
    ...
    qSort(memList.begin(), memList.end(), subDevListSortContr);
...
void CDlgTalk_qt::confCompere_inviteToSpeak() {
    ...
    if (idInfo_sel.ui64Id == pMisCnt->idInfo.ui64Id) {
        ...
        if (pMgrVar->av.taskInfo.ucbStarter) {
            ...
            //
            confOthers_requestToSpeak(hMgr, pMgrVar->av.iIndex_sharedObj_localAv, null, TRUE);
