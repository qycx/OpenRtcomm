...
//qint64 lastTim = 0;
bool CDlg2ndScreen::nativeEvent(const QByteArray& eventType, void* message, long* result)
{
    Q_UNUSED(eventType);
    ...
    case  CONST_qyWm_comm:
    {
        ...
        if (pComm->uiType == CONST_misMsgType_procVideo_qmc) {
            ...
            if (m_streamInfo.m_iW_scroll != w || m_streamInfo.m_iH_scroll != h) {
                ...
                //
                freeCapImages(hDlg, m_var.hCtrl, &m_var.images, null, &m_var.hDc);
...
void CDlg2ndScreen::on_timer_winMethod()
{
    ...
    if (!n) {
        if (m_streamInfo.m_bExists_stream) {
            freeCapImages(m_hWnd, m_var.hCtrl, &m_var.images, null, &m_var.hDc);
...
void CDlg2ndScreen::clearScrollBar()
{
    ...
    //if (pm_var->av.taskInfo.bTaskExists) 
    {
        ...
        //
        freeCapImages(hDlgTalk, m_var.hCtrl, &m_var.images, null, &m_var.hDc);
