...
void WinContactsList::autoSet() {

	//ui->treeWidgetContacts->setItemSelected(m_pRootFriendItemGroup, true);
	ui->treeWidgetContacts->expandItem(m_pRootFriendItemGroup);

	for (int i = 0; i < m_pRootFriendItemGroup->childCount(); ++i) {
		QString name = m_pRootFriendItemGroup->child(i)->text(0);
		//name = name;
		//pChild->setData(0, Qt::UserRole + 1, QVariant::fromValue(user));

		QVariant data = m_pRootFriendItemGroup->child(i)->data(0, Qt::UserRole + 1);
		
		if (data.canConvert<WinObjUser>())
		{
			WinObjUser user = data.value<WinObjUser>();
			QString name = user.name;

			if (name == "test") {
				ui->treeWidgetContacts->setItemSelected(m_pRootFriendItemGroup->child(i), true);
				ui->treeWidgetContacts->itemClicked(m_pRootFriendItemGroup->child(i), 0);			
...
void WinContactsList::addTmpContactItem(int id)
{
	...
	QString username;
	idinfo.ui64Id = id;
	getTalkerDesc(idinfo, &uiObjType, grpName_from, mycountof(grpName_from), null, null, 0, null, 0, srcName_from,mycountof(srcName_from));
...
{
	QString info_start = info1.dw + info1.bm + info1.syr + info1.userId;
	QString info_end = info2.dw + info2.bm + info2.syr + info2.userId;
