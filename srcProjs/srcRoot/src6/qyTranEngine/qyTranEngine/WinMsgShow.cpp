...
void MsgHtmlObj::fileRecv(QString msgid)
{
    ...
    *pT1 = 0;
    __int64 tStartTime;
    tStartTime = _tcstoui64(pT, null, 10);
    pT1++;
    ...
    dlgTalk_procTask1(m_hTalk, iTaskId, CONST_imOp_recv_accept, null, &param);
...
void MsgHtmlObj::clickHeader(int userid,int chatType) {
    ...
    if (userid != 0) {
        ...
        if (chatType == 1) {
            ...
            CDlgTalk_qt* cdlgTalkqt = null;
            cdlgTalkqt = (CDlgTalk_qt*)QWidget::find((WId)m_hTalk);
            getTalkerDesc(infoId, null, null, null, null, null, null, null, null, idinfoto_name, mycountof(idinfoto_name));
...
void MsgHtmlObj::fileCancel(QString msgid)
{
    ...
    if (messFind[0].fromUserId == QString::number(pMisCnt->idInfo.ui64Id)) {
        dlgTalk_procTask1(m_hTalk, iTaskId.toInt(), CONST_imOp_send_cancel, null, &param);
    }
    else {
        dlgTalk_procTask1(m_hTalk, iTaskId.toInt(), CONST_imOp_recv_cancel, null, &param);
    }
...
void MsgHtmlObj::openFile(QString fileName, QString msgid, QString userid) {
    ...
    if (fileName.isEmpty()) {
        ...
        pathStr = QString::fromStdWString(std::wstring(pDir)) + fileFind[0].content.remove(QRegExp("^ +\\s*"));;
    }
    else {
        pathStr = QString::fromStdWString(std::wstring(pDir)) + fileName.remove(QRegExp("^ +\\s*"));;
    }
