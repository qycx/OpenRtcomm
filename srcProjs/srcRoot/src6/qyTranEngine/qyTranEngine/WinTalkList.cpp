...
void WinTalkList::on_listWidgetMsg_itemClicked(QListWidgetItem* item)
{
    ...
    CMainFrame* pMainWnd_a = (CMainFrame*)getObjAddr(hMainWnd);
    ...
    if (pContactItem)
    {
        ...
        pContactItem->Clear();
        pMainWnd_a->UpMsgCount(msgCount);
...
void WinTalkList::updateTalkItem(qint64 idInfo, unsigned  short status) 
{
    ...
    getTalkerDesc(ui_idInfo, &uiObjType, null, null, null, null, null, null, null, null, null);
...
void WinTalkList::slot_recv_new_message(SendData sd)
{
    ...
    getTalkerDesc(addr_logicalPeer_idInfo, &uiObjType_peer, grpName_peer, mycountof(grpName_peer), &idInfo_creator, null, 0, null, 0, srcName_peer, mycountof(srcName_peer));
   
    getTalkerDesc(idInfo_from, &uiObjType_from, grpName_from, mycountof(grpName_from), &idInfo_creator, null, 0, null, 0, srcName_from, mycountof(srcName_from));
...
void WinTalkList::addListWidget(WinObjUser user)
{
    ...
    QY_MESSENGER_ID idInfo_creator;
    getTalkerDesc(idinfo, &uiObjType, grpName_from, mycountof(grpName_from), &idInfo_creator, null, 0, null, 0, srcName_from, mycountof(srcName_from));
    QString head = "";
