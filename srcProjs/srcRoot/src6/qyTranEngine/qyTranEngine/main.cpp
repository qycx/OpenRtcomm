...
int main(int argc, char* argv[])
{
	...
	//ui64FileLen:4478906368,uiSizePerSnd_internal:921600,uiDataBlockIndex:4860
	unsigned long long uiFileLen = 4478906368;
	unsigned int uiDataBlockIndex = 4860;
	unsigned int uiSizePerSnd_internal = 921600;
	unsigned long long ulDataBlockIndex = 4850;
	unsigned long long last = ulDataBlockIndex * uiSizePerSnd_internal;
	if (ulDataBlockIndex * uiSizePerSnd_internal > uiFileLen) {

		unsigned  int uiBlocks = 0;
	}
	else {
		unsigned  int uiBlocks = (uiFileLen - ulDataBlockIndex * uiSizePerSnd_internal - 1) / uiSizePerSnd_internal + 1;
...
#ifdef IS_UPLOADFILE
	dlgUploadFile* dlgUpLoad = nullptr;
#endif
	...
	if (InitInstance(nullptr, 0) == false)goto  errLabel;  
	{
		...
		for (i=0;i<maxCnt;i++ ) {
			...
			//
			int iTickCnt0 = myGetTickCount(null);

			
#ifdef IS_UPLOADFILE
			dlgUpLoad = new dlgUploadFile();
			if (dlgUpLoad == nullptr) goto errLabel;
#endif 
			...
			//
			int  iTickCnt1 = myGetTickCount(null);
			...
			int iTickCnt2 = myGetTickCount(null);
			...
			//cmainFrame->switchToContact();
			//cmainFrame->setFixedSize(QSize(0, 0));
			//cmainFrame->hide();
#endif 
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	g_pQyMc->eProgType = ProgType_TranEngine;
	//
