...
int viewInviteHint(HWND  hDlgTalk)
{
	...
	CDlgTalk_qt* pTalk = (CDlgTalk_qt*)getObjAddr(hDlgTalk);
	...
	QRect rect = video_cdlgTalkqt->geometry();
	////计算显示原点
     int x = rect.x() + rect.width() / 2 - d_WIDTH / 2;
	int y = rect.y() + rect.height() / 2 - d_HEIGHT / 2;
	dlg->move(x, y);
	dlg->show();
...
int  closeInviteHint()
{
	...
	if (IsWindow(var.hintTaskStatus.hTool_dlgHintAccept)) {
		...
		//
		var.hintTaskStatus.hTool_dlgHintAccept = null;
	}
