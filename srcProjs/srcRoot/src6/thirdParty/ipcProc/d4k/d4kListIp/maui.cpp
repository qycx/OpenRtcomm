...
int _tmain(int argc, TCHAR* argv[])
{
    ...
    try {
        ...
        do {
            ...
            if (initShowInfo_func(0, _T("syncQ_showInfo_d4k"), errHintWndCls, false, true, (char*)"127.0.0.1", bCli, &gBuf_dhListIp.m_var.pShowInfoStruct)) {
                //goto  errLabel;
                break;
            }
            ...
            //
            set_who_showInfo(_T("d4kListIp"));

            //
            if (gBuf_dhListIp.m_var.appParams.nvrIp[0] == 0) {
                showInfo_open0(0, 0, _T("d4kListIp: nvrIp is null, failed"));
                break;
            }
        ...
        //
            ...
            //
            int pret = initQisPipe(&qCfg, pipeName, false, _T("d4k.listIp"), &param, gBuf_dhListIp.m_var.pQisPipe);


            printLogChar("----------nvrIp------begin--------");
            printLogChar(gBuf_dhListIp.m_var.appParams.nvrIp);
            printLogChar("----------nvrIp------end--------");

            /*
            TcpClient client(gBuf_dhListIp.m_var.appParams.nvrIp, 61111);
            ...
            for (; ; ) {
                ...
                if (gBuf_dhListIp.m_var.m_bAutoClose_ifToolLongNoRecvd_dh) {
                    ...
                    if (abs(iDiffInMs) > CONST_toInMs_ipcProc) {
                        //
                        showInfo_open0(0, _T("d4kListIp"), _T("too long to recv, quit now\n"));
                        //
                        break;
                    }
                }
    ...
    {
        printf("exception\n");
        printLogChar("exception");

        exit(0);
    }

    //
    printf("d4kListIp finished\n");
    printLogChar("----------d4kListIp------finished--------");
