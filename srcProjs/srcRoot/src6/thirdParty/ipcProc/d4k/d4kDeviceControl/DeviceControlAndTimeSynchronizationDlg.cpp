	...
	: CDialog(CDeviceControlAndTimeSynchronizationDlg::IDD, pParent)
{
	...
	//
		//
	m_strDvrUserName = gBuf_dhDeviceControl.m_var.appParams.nvrUsr;
	m_strDvrPwd = gBuf_dhDeviceControl.m_var.appParams.nvrPwd;
...
BOOL CDeviceControlAndTimeSynchronizationDlg::OnInitDialog()
{
	...
	//
	//
	DWORD dwNvrIp = inet_addr(gBuf_dhDeviceControl.m_var.appParams.nvrIp);
	dwNvrIp = ntohl(dwNvrIp);
	m_dvrIP.SetAddress(dwNvrIp);

	//
	PostMessage(WM_COMMAND, MAKEWPARAM(IDC_BTN_LOGIN, BN_CLICKED), NULL);
...
void CDeviceControlAndTimeSynchronizationDlg::OnBtnLogin() 
{
	...
	if(bValid)
	{
		...
		TCHAR *pchUserName=(LPTSTR)(LPCTSTR)m_strDvrUserName;
		TCHAR *pchPassword=(LPTSTR)(LPCTSTR)m_strDvrPwd;
		...
		if(0 == m_LoginID)
		{
			...
			showInfo_open0(0, 0, _T("deviceControl.OnBtnLogin: bErr true"));
		}
		else
		{
...
void CDeviceControlAndTimeSynchronizationDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	//
	if (gBuf_dhDeviceControl.m_var.hMainWnd != m_hWnd) {
		gBuf_dhDeviceControl.m_var.hMainWnd = m_hWnd;
	}

	//
	if (gBuf_dhDeviceControl.m_var.bErr) {
		showInfo_open0(0, 0, _T("deviceControl.timer: bErr is true, so close wnd now"));
		//
...
int printLogChar(const char* log) {
	//return 0;
	FILE* pf = fopen("D:/qycx/devicecontrol.txt", "a+");
	if (pf == NULL)
	{
		perror("打开文件失败！");
		return 1;
...
#include <sstream>
BOOL CDeviceControlAndTimeSynchronizationDlg::OnCopyData(CWnd* pWnd, COPYDATASTRUCT* pCopyDataStruct)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	OnvifMsg_nvrControl* pNc;
	pNc = (OnvifMsg_nvrControl*)pCopyDataStruct->lpData;
	std::stringstream ss;
	ss << "cmd:" << pNc->usCmd << ",iChannel:" << pNc->iChannel;
	printLogChar("------------------OnCopyData------------begin---------------");
	printLogChar(ss.str().c_str());
	printLogChar("------------------OnCopyData------------end---------------");
		...
		switch (pNc->usCmd) {
				case  CONST_nvrControlCmd_selectChannel:
					...
					if (0 != m_LoginID)
					{
						...
						switch (pNc->iChannel) {
						case  1:
							type = DH_KEYBOARD_BTN1;
							break;
						case  2:
							type = DH_KEYBOARD_BTN2;
							break;
						case  3:
							type = DH_KEYBOARD_BTN3;
							break;
						case  4:
							type = DH_KEYBOARD_BTN4;
							break;
						case  5:
							type = DH_KEYBOARD_BTN5;
							break;
						case  6:
							type = DH_KEYBOARD_BTN6;
							break;
						case  7:
							type = DH_KEYBOARD_BTN7;
							break;
						case  8:
							type = DH_KEYBOARD_BTN8;
							break;
						case  9:
							type = DH_KEYBOARD_BTN9;
							break;
						case  10:
							type = DH_KEYBOARD_BTN10;
							break;
						case  11:
							type = DH_KEYBOARD_BTN11;
							break;
						case  12:
							type = DH_KEYBOARD_BTN12;
							break;
						case  13:
							type = DH_KEYBOARD_BTN13;
							break;
						case  14:
							type = DH_KEYBOARD_BTN14;
							break;
						case  15:
							type = DH_KEYBOARD_BTN15;
							break;
						case  16:
							type = DH_KEYBOARD_BTN16;
							break;
							//						
						default:
							type = DH_KEYBOARD_SPLIT;
							break;
						}
						//
						//
						BOOL bSuccess = CLIENT_ControlDevice(m_LoginID, type, NULL, DEMO_SDK_WAITTIME);
						if (!bSuccess)
						{
							//MessageBox(ConvertString("Control failed!"), ConvertString("Prompt"));
							showInfo_open0(0, 0, _T("Control failed"));
							break;
						}
