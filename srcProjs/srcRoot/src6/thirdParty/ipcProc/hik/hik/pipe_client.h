...
public:
	...
	bool Connect() {
		...
		if (WaitNamedPipe(wstr.c_str(), NMPWAIT_WAIT_FOREVER) == FALSE) {
		...
		std::cout << "open pipe: " << pipe_name_ << std::endl;
		hPipe_ = CreateFile(wstr.c_str(), GENERIC_READ | GENERIC_WRITE, 0,
	...
	void Write(const char* content) {
		...
		printLogChar("-------pipe write----------in------------------");

		if (state_ != PipeClientState_open) {
			return;
		}

		printLogChar("-------pipe write----------begin------------------");
