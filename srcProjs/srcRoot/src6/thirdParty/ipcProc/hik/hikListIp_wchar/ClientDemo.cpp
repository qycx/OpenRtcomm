...
Return:	
**********************************************************/
wchar_t* CharToTCHAR(char* pChar)
{
...
BOOL CClientDemoApp::InitInstance()
{
	...
	printLogChar("---------------InitInstance-------------111111111111-----------------");
...
//     _CrtSetBreakAlloc(21094);
	...
	printLogChar("---------------InitInstance-------------22222222222-----------------");

    g_hDSSDK = LoadLibrary(_T("DsSdk.dll"));
	...
	{
		DWORD dwErrCode = NET_DVR_GetLastError();
		TCHAR szErrLan[128] = { 0 };
		swprintf_s(szErrLan, _T("ErrCode:%d"), dwErrCode);
		AfxMessageBox(szErrLan);
		return FALSE;
	}

	printLogChar("---------------InitInstance-------------33333333333-----------------");
...
//     }
	...
	printLogChar("---------------InitInstance-------------44444444444-----------------");
    ...
    {
        OutputDebugString(_T("Set NET_DVR_LOCAL_CFG_MESSAGE_CALLBACK_V51 failed\n"));
    }
    else
    {
        OutputDebugString(_T("Set NET_DVR_LOCAL_CFG_MESSAGE_CALLBACK_V51 successful\n"));
    }

	printLogChar("---------------InitInstance-------------55555555555-----------------");
	...
	do {
		...
		if (initShowInfo_func(0, _T("syncQ_showInfo_hik"), errHintWndCls, false, true, (char*)"127.0.0.1", bCli, &gBuf_dhListIp.m_var.pShowInfoStruct)) {
			//goto  errLabel;
			break;
		}
		...
		//
		{
			...
			if (gBuf_dhListIp.m_var.appParams.nvrIp[0] == 0) {
				showInfo_open0(0, 0, _T("dhHik: nvrIp is null, failed"));
				break;
			}
		...
		//
			...
			//printLogChar(pipeName);
			//
			PARAM_initQisPipe  param = { 0 };
			param.pf_onRead = dhListIp_qisPipe_onRead;
			//
			param.uiMaxToInMs_read = CONST_toInMs_pipe_read;
			qCfg.uiMaxCnt_semaTrigger = 0;


			int ret = initQisPipe(&qCfg, pipeName, false, _T("hik.listIp"), &param, gBuf_dhListIp.m_var.pQisPipe);
		...
		//
		bool b = gBuf_dhListIp.m_var.m_bAutoClose_ifToolLongNoRecvd_dh;

		printLogChar("---------------InitInstance-------------66666666666666-----------------");

		//NET_DVR_SetConnectTime(3000, 3);
		CClientDemoDlg dlg(gBuf_dhListIp.m_var.appParams.nvrIp, gBuf_dhListIp.m_var.appParams.nvrUsr, 
			gBuf_dhListIp.m_var.appParams.nvrPwd);
		m_pMainWnd = &dlg;

		printLogChar("---------------InitInstance-------------777777777777777777-----------------");

		try
		{
			dlg.DoModal();
			printLogChar("---------------InitInstance-------------8888888888888888-----------------");
		}
		catch (...)
		{
			printLogChar("---------------InitInstance-------------99999999999999999-----------------");
			return FALSE;
		}
	...
	printLogChar("---------------InitInstance-------------000000000000000-----------------");
...
void GetCurrentPath(TCHAR *pPath)
{
	...
	wcsncpy(pPath, strPath.Left(iPos).GetBuffer(0), iPos);
	pPath[iPos]='\0';
}
...
DWORD CStringTodwIP(CString strIP)
{
	...
	strIPTemp.Format(_T("%s"),strIP.GetBuffer(0));
	strRight = strIPTemp;
	...
	while (iPos>0)
	{
		strLeft.Format(_T("%s"),strRight.Left(iPos));
		dwRet=(dwRet<<8) + (unsigned)_wtoi(strLeft);
		strRight.Format(_T("%s"),strRight.Right(iLen - iPos - 1));
		iLen=strRight.GetLength();
		iPos=strRight.Find(_T("."),0);
	}
	dwRet=(dwRet<<8) + (unsigned)_wtoi(strRight);
	return dwRet;
}
...
void g_CreateLocalDir()
{
	...
	{
		csDir.Format(_T("%s\\"), g_struLocalParam.chDownLoadPath);
	...
	{
		csDir.Format(_T("%s\\"), g_struLocalParam.chPictureSavePath);
	...
	{
		csDir.Format(_T("%s\\"), g_struLocalParam.chRemoteCfgSavePath);
...
{	//check IP address correct
	...
	{
		swscanf(str,_T("%d.%d.%d.%d"),&a3,&a2,&a1,&a0);
...
void g_ReadLocalReadSensorAdd()
{
    ...
    csFilename.Format(_T("%s\\LocalSensorAdd.dat"), g_szWorkDir);
