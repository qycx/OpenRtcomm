...
#define ALARM_OUT_FORMAT		_T("AlarmOut%d")
#define ALARM_IN_NAME			_T("AlarmIn%d")
#define IP_ALARM_IN_NAME		_T("IPAlarmIn%d")
#define IP_ALARM_OUT_NAME		_T("IPAlarmOut%d")
#define IP_ALARM_OUT_BYCHAN		_T("%s-%d")
#define ANALOG_C_FORMAT			_T("Camera%d")
#define IP_CAMERA_NAME			_T("IPCamera%02d")
#define DIGITAL_C_FORMAT		_T("IPCamera%d")
#define MIRROR_C_FORMAT			_T("Mirror%d")
...
typedef struct STRU_CHANNEL_INFO{ 
	...
	TCHAR	chChanName[100];	//channel name
	...
	TCHAR	chAccessChanIP[16];//ip addr of IP channel
...
typedef struct STRU_LOCAL_PARAM
{
	...
	TCHAR		chDownLoadPath[100];		//remote file download directory
	TCHAR		chPictureSavePath[100];		//image capture directory
	TCHAR		chRemoteCfgSavePath[100];	//remote config file saving directory
	TCHAR		chClientRecordPath[100];		//client record path
	...
	STRU_LOCAL_PARAM()
	{
		...
		swprintf(chDownLoadPath, _T("%s"), _T("C:\\DownLoad"));
		swprintf(chPictureSavePath, _T("%s"), _T("C:\\Picture"));
		swprintf(chRemoteCfgSavePath ,_T("%s"), _T("C:\\SaveRemoteCfgFile"));
		swprintf(chClientRecordPath ,_T("%s"), _T("C:\\mpeg4record\\2008-04-30"));
		swprintf(chIPServerIP ,_T("%s"), _T("172.7.28.123"));
