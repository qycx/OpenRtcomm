...
void AddDevice(CString m_csDevAddr, int m_nLoginPort, CString m_csUserName, CString m_csPwd, CString m_csNodeName, int m_iDeviceIndex)
{
    char szDeviceIP[128] = { 0 };
    char cUserName[100] = { 0 };
    char cPassword[100] = { 0 };
    ...
    if (strncmp(m_csNodeName.GetBuffer(0), "All", 3) == 0)//Use "All.X" X means the device number，From 192.168.1.Y To 192.168.1.X+Y
    {
        ...
        for (int i = 0; i < nErr; i++)
        {
            ...
            g_struDeviceInfo[iDeviceIndex].iDeviceChanNum = 1;
            g_struDeviceInfo[iDeviceIndex].lDevicePort = m_nLoginPort;
            g_struDeviceInfo[iDeviceIndex].byHttps = 0;//m_cmbHttps.GetCurSel();    //0-http 1-https 2-自适应
            g_struDeviceInfo[iDeviceIndex].byLoginMode = 0;// m_cmbLoginMode.GetCurSel();  //0-private 1-ISAPI 2-自适应
            g_struDeviceInfo[iDeviceIndex].byVerifyMode = 0;// m_cmbVerifyMode.GetCurSel();  //0-不认证 1-双向认证 2-单向认证
            sprintf(g_struDeviceInfo[iDeviceIndex].chLocalNodeName, "%s", g_struDeviceInfo[iDeviceIndex].chDeviceIP);
            g_pMainDlg->RedrawDeviceNode(iDeviceIndex + 1000);
        }
    ...
    memcpy(struLoginInfo.sPassword, cPassword, PASSWD_LEN);
    struLoginInfo.wPort = m_nLoginPort;
    struLoginInfo.byProxyType = 0;// m_cmbProxyType.GetCurSel();
    g_struDeviceInfo[m_iDeviceIndex].byProxyType = struLoginInfo.byProxyType;
    ...
    */
    //struLoginInfo.byUseTransport = 1;
    //struLoginInfo.byRes3[119] = 2;
    struLoginInfo.byHttps = 0;//m_cmbHttps.GetCurSel();    //0-http 1-https 2-自适应
    struLoginInfo.byLoginMode = 0;// m_cmbLoginMode.GetCurSel();  //0-private 1-ISAPI 2-自适应
    struLoginInfo.byVerifyMode = 0;//  m_cmbVerifyMode.GetCurSel();
    ...
    sprintf(g_struDeviceInfo[m_iDeviceIndex].chLocalNodeName, "%s", m_csNodeName.GetBuffer(0));
    g_struDeviceInfo[m_iDeviceIndex].lDevicePort = m_nLoginPort;
    g_struDeviceInfo[m_iDeviceIndex].byHttps = 0;// m_cmbHttps.GetCurSel();    //0-http 1-https 2-自适应
    g_struDeviceInfo[m_iDeviceIndex].byLoginMode = 0;// m_cmbLoginMode.GetCurSel();  //0-private 1-ISAPI 2-自适应
    g_struDeviceInfo[m_iDeviceIndex].byVerifyMode = 0;// m_cmbVerifyMode.GetCurSel();  //0-不认证 1-双向认证 2-单向认证
    g_struDeviceInfo[m_iDeviceIndex].bySupport1 = struDeviceInfo.bySupport1;
    g_struDeviceInfo[m_iDeviceIndex].bySupport2 = struDeviceInfo.bySupport2;
    g_struDeviceInfo[m_iDeviceIndex].bySupport4 = struDeviceInfo.bySupport4;
    g_struDeviceInfo[m_iDeviceIndex].bySupport7 = struDeviceInfo.bySupport7;
