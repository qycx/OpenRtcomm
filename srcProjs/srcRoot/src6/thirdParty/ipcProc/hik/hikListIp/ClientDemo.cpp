...
void threadFunction() {
	...
	while (1) {
		...
		auto elapsed_minutes = std::chrono::duration_cast<std::chrono::minutes>(end - start).count();

		/*
		if (elapsed_minutes >= 5) {
			showInfo_open0(0, 0, _T("hikListIp_timeout exit"));
			exit(0);
		}
		else {
		...
		*/
		...
		if (abs(iDiffInMs) > CONST_toInMs_ipcProc * 2) {
			//PostMessage(hwndDlg, WM_MY_CLOSE_MSG, 0, 0);
			printLogCharSpec("programe exit");
			exit(0);
...
BOOL CClientDemoApp::InitInstance()
{
	...
	do {
		...
		//showInfo_open0(0, 0, _T("hikListIp init"));
		...
		//
			...
			param.pf_onRead = hikListIp_qisPipe_onRead;
			//
			...
			int ret = initQisPipeA("qisPipe", "qisPipe", 100, pipeName, false, _T("hik.listIp"), &param, gBuf_dhListIp.m_var.pQisPipe);
			if (ret) {
				OutputDebugString(_T("initInstance: initQisPipe failed\n"));
				//ret 1
				break;
			}
		...
		printLogChar("---------------InitInstance-------------6-----------------");

		/*
		DWORD processId = GetCurrentProcessId();
		std::stringstream ss;
		ss << "pipe_list_" << processId;
		printLogChar(ss.str().c_str());
		g_pipe_server = new Pipe_Server(ss.str(), std::bind(msgProcess, std::placeholders::_1));
		g_pipe_server->Start();
		*/
...
#ifndef _DEBUG
		printLogCharSpec("start check thread");
		std::thread myThread(threadFunction);
#endif
		

		//g_pipeClient = new Pipe_Client("pipe_hik");
		//g_pipeClient->Start();
		...
		printLogChar("---------------InitInstance-------------7-----------------");

		try
		{
			dlg.DoModal();
			printLogChar("---------------InitInstance-------------8-----------------");
		}
		catch (...)
		{
			printLogChar("---------------InitInstance-------------9-----------------");
			return FALSE;
		}
