...
public:
	CClientDemoDlg(CString csDevAddr, CString csUserName, CString csPwd, CWnd* pParent = NULL);   // standard constructor
	virtual ~CClientDemoDlg();
	int m_iSelListType;

	CString m_csDevAddr;
	int m_nLoginPort = 8000;
	CString m_csUserName;
	CString m_csPwd;

	void SetRemoteKeyHwnd(HWND hwnd);
...
public:
	...
	afx_msg LRESULT OnMyCloseMessage(WPARAM wParam, LPARAM lParam);
	afx_msg LRESULT OnSetRemoteMessage(WPARAM wParam, LPARAM lParam);
...
//0.operation on main interface
	...
	BOOL DoPreset(LONG lChannel, DWORD dwPTZPresetCmd, DWORD dwPresetIndex);
	void RefreshIPDevLocalCfg(int iDeviceIndex);
	void RefreshIPDevLocalCfg(int iDeviceIndex, LONG lUserID);
...
//1.device list
	...
	BOOL DoGetDeviceResoureCfg(int iDeviceIndex, LONG lUserID, int iGroupNO);
	void DblAllTree();
	...
	void Process3DLocate(OnvifMsg_ptz* pPtz);
...
// 	3.configuration buttons
	...
	int GetDeviceType(LONG lUserID, NET_DVR_DEVICEINFO_V40& deviceInfo);
	...
	void ProcFacecaptureStatistics(WPARAM wParam, LPARAM lParam);//抓拍统计上传
