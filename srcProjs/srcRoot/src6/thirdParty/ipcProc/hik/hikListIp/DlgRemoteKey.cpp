...
CDlgRemoteKey::CDlgRemoteKey(CClientDemoDlg* clientDlg, CWnd* pParent /*=NULL*/)
...
BOOL CDlgRemoteKey::OnInitDialog() 
{
	...
	if (m_clientDlg)
		m_clientDlg->SetRemoteKeyHwnd(GetSafeHwnd());
		//::SendMessage(m_clientDlg->GetSafeHwnd(), WM_SET_REMOTE_MESSAGE, reinterpret_cast<WPARAM>(GetSafeHwnd()), 0);
...
BOOL CDlgRemoteKey::OnCopyData(CWnd* pWnd, COPYDATASTRUCT* pCopyDataStruct)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值

	printLogChar("---------OnCopyData----------------in-------------");
	std::stringstream ss;
	...
	// 存储在int变量中
	int timestamp_int = static_cast<int>(timestamp);

	int duration = timestamp_int - m_lastTime;
	if (m_lastTime == 0 || duration >= 2) {
		//记录请求时间
		printLogChar("----------------true-------------");

		m_lastTime = static_cast<int>(timestamp);
	}
	else {
		//过滤频繁请求
		return false;
	...
	ss << "OnCopyData:usCmd=" << pNc->usCmd << ",iChannel=" << pNc->iChannel << ",ServerID=" << m_lServerID;

	printLogChar(ss.str().c_str());

	TCHAR  tBuf[128];
	_sntprintf(tBuf, mycountof(tBuf), _T("CDlgRemoteKey OnCopyData:usCmd=%u,iChannel=%d,ServerID=%d"), pNc->usCmd, pNc->iChannel, m_lServerID);
	showInfo_open0(0, _T("hikListIP"), tBuf);
	...
	do {
		...
		case  CONST_nvrControlCmd_selectChannel:
			...
			if (m_lServerID >= 0)
			{
				...
				BOOL bSuccess = NET_DVR_ClickKey(m_lServerID, keyCode);
				if (!bSuccess)
				{
					//MessageBox(ConvertString("Control failed!"), ConvertString("Prompt"));
					showInfo_open0(0, _T("hikListIP"), _T("Control failed"));
					printLogChar("hikListIP:Control failed");
					break;
				}
