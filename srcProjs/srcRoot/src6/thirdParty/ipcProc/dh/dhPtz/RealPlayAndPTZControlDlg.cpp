	...
	: CDialog(CRealPlayAndPTZControlDlg::IDD, pParent)
{
	...
	//
	m_DvrUserName = gBuf_dhPtz.m_var.appParams.nvrUsr;
	m_DvrPassword = gBuf_dhPtz.m_var.appParams.nvrPwd;
}
...
BOOL CRealPlayAndPTZControlDlg::OnInitDialog()
{
	...
	//
	DWORD dwNvrIp = inet_addr(gBuf_dhPtz.m_var.appParams.nvrIp);
	dwNvrIp = ntohl(dwNvrIp);
	m_DvrIPAddr.SetAddress(dwNvrIp);

	//


	//
	PostMessage(WM_COMMAND, MAKEWPARAM(IDC_BT_Login, BN_CLICKED), NULL);
...
void CRealPlayAndPTZControlDlg::IsValid()
{
	if(g_share_hWnd)
	{
		...
		showInfo_open0(0, 0, _T("Only one program will be allowed to open!"));
		//
#if 0
...
void CRealPlayAndPTZControlDlg::OnBTLogin() 
{
	...
	if(bValid)
	{
		...
		pchDVRIP = (LPTSTR)(LPCTSTR)strDvrIP;
		WORD wDVRPort=(WORD)m_DvrPort;
		TCHAR *pchUserName=(LPTSTR)(LPCTSTR)m_DvrUserName;
		TCHAR *pchPassword=(LPTSTR)(LPCTSTR)m_DvrPassword;

		NET_IN_LOGIN_WITH_HIGHLEVEL_SECURITY stInparam;
		memset(&stInparam, 0, sizeof(stInparam));
		stInparam.dwSize = sizeof(stInparam);
		//strncpy(stInparam.szIP, pchDVRIP, sizeof(stInparam.szIP) - 1);
		myTChar2Utf8(pchDVRIP, stInparam.szIP, sizeof(stInparam.szIP) - 1);
		//strncpy(stInparam.szPassword, pchPassword, sizeof(stInparam.szPassword) - 1);
		myTChar2Utf8(pchPassword, stInparam.szPassword, sizeof(stInparam.szPassword) - 1);
		//strncpy(stInparam.szUserName, pchUserName, sizeof(stInparam.szUserName) - 1);
		myTChar2Utf8(pchUserName, stInparam.szUserName, sizeof(stInparam.szUserName) - 1);
		...
		if(0 != lRet)
		{
			...
			//  点击播放按钮
			PostMessage(WM_COMMAND, MAKEWPARAM(IDC_BUTTON_Play, BN_CLICKED), NULL);
		...
		else
		{
			...
			showInfo_open0(0, 0, _T("ptz.OnBTLogin: bErr true"));
		}
	}
	SetWindowText(ConvertString("RealPlayAndPTZControl"));
}
...
{
	SetDlgItemText(IDC_PTZSTATUS,_T(""));
	if (0 == m_LoginID)
	{
		return;
	}
...
void CRealPlayAndPTZControlDlg::DirectPlayMode(int iDispNum,int iChannel,HWND hWnd)
{
	...
	else
	{
		...
		showInfo_open0(0, 0, _T("DirectPlayMode:Fail to play!"));
	}
}
...
{	
	...
	if(bOpenRet)
	{
		...
		{           
			...
			{
				//MessageBox(ConvertString("Fail to play!"), ConvertString("Prompt"));
				showInfo_open0(0, 0, _T("ServerPlayMode:Fail to play!"));
...
void CRealPlayAndPTZControlDlg::MultiPlayMode(int iDispNum, HWND hWnd)
{
	...
	{
		//MessageBox(ConvertString("Fail to play!"), ConvertString("Prompt"));
		showInfo_open0(0, 0, _T("MultiPlayMode:Fail to play!"));
	} 
}
...
BOOL CRealPlayAndPTZControlDlg::SetMultiSpliteMode(int nChannel, int nMultiPlay)
{
	...
	if (0 != m_LoginID)
	{
		...
		if (CFG_EM_PREVIEW_MODE_SPLITSNAP == emPreviewMode)
		{
			...
			{
				CString csErr;
				csErr.Format(_T("CLIENT_SetSplitMode failed:%d"), CLIENT_GetLastError() & 0x7ffffff);
	...
	CString csErr;
	csErr.Format(_T("Please login first !"));
	MessageBox(ConvertString(csErr), ConvertString("Prompt"));
	return FALSE;
}
...
void CRealPlayAndPTZControlDlg::MultiPlayServerMode(int iDispNum, HWND hWnd)
{
	...
	else
	{
		...
		g_PlayAPI.PLAY_CloseStream(iDispNum);
		//MessageBox(ConvertString("Fail to play!"), ConvertString("Prompt"));
		showInfo_open0(0, 0, _T("MultiPlayServerMode:Fail to play!"));
	}
}
...
void CRealPlayAndPTZControlDlg::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	OutputDebugString(_T("timer\n"));

	//
	if (gBuf_dhPtz.m_var.hMainWnd != m_hWnd) {
		gBuf_dhPtz.m_var.hMainWnd = m_hWnd;
	}

	if (gBuf_dhPtz.m_var.bErr1) {
		showInfo_open0(0, 0, _T("ptz.timer: bErr is true, so close wnd now"));
		//
		PostMessage(WM_CLOSE);
	}

	//
	if (gBuf_dhPtz.m_var.m_bAutoClose_ifToolLongNoRecvd_qm) {
		//
		DWORD  dwTickCnt = myGetTickCount(nullptr);
		int iDiffInMs = dwTickCnt - gBuf_dhPtz.m_var.dwLastTickCnt_recvd;
...
#define AREA_ADUST 25
bool CRealPlayAndPTZControlDlg::Exec3dLocate(OnvifMsg_ptz* pPtz, int curChannel)
{
	TCHAR  tBuf[256];

	_sntprintf(tBuf, mycountof(tBuf), _T("Exec3dLocate: channel %d, ptz cmd %d, wh %dX%d, top %d,%d, bottom %d,%d"), 
		(int)pPtz->iChannel, (int)pPtz->ucCmd,
		(int)pPtz->paramU.param3ddw.sW_video, (int)pPtz->paramU.param3ddw.sH_video,
		(int)pPtz->paramU.param3ddw.xTop, (int)pPtz->paramU.param3ddw.yTop, (int)pPtz->paramU.param3ddw.xBottom, 
		(int)pPtz->paramU.param3ddw.yBottom);

	showInfo_open0(0, _T("RealPlayAnd"), tBuf);
	//printLog(tBuf);

	CRect WinRect(0,  0, pPtz->paramU.param3ddw.sW_video, pPtz->paramU.param3ddw.sH_video);
	CPoint pointStart(pPtz->paramU.param3ddw.xTop - AREA_ADUST, pPtz->paramU.param3ddw.yTop - AREA_ADUST);
	CPoint pointEnd(pPtz->paramU.param3ddw.xTop + AREA_ADUST, pPtz->paramU.param3ddw.yTop + AREA_ADUST);
	...
	if (width == 0 || height == 0) {
		showInfo_open0(0, _T("RealPlayAnd"), _T("error: width or height == 0"));
		return false;
	}
	...
	_sntprintf(tBuf, mycountof(tBuf), _T("Exec3dLocate: param1=%d, param2=%d, param3=%d, curChannel=%d"), param1, param2, param3, curChannel);

	showInfo_open0(0, _T("RealPlayAnd"), tBuf);

	BOOL bRet = CLIENT_DHPTZControlEx2(m_LoginID, curChannel - 1, DH_EXTPTZ_FASTGOTO, param1, param2, param3, FALSE, (void*)param4);
	if (bRet) {
		showInfo_open0(0, _T("RealPlayAnd"), _T("Exec3dLocate: CLIENT_DHPTZControlEx2 exec success"));
	}
	else {
		showInfo_open0(0, _T("RealPlayAnd"), _T("Exec3dLocate: CLIENT_DHPTZControlEx2 exec failed"));
	}
...
BOOL CRealPlayAndPTZControlDlg::OnCopyData(CWnd* pWnd, COPYDATASTRUCT* pCopyDataStruct)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值

	OnvifMsg_ptz* pPtz;
	pPtz  =  (OnvifMsg_ptz  *  )pCopyDataStruct->lpData;
	...
	do {
		...
		int nCur = this->m_comboChannel.GetCurSel();
		int curChannel = nCur + 1;
		if (curChannel != pPtz->iChannel) {
			//
			showInfo_open0(0, 0, _T("curChannel is not matched, change it"));
			//
			this->m_comboChannel.SetCurSel(pPtz->iChannel - 1);
			//
			this->OnBUTTONPlay();
		}
		...
		_sntprintf(tBuf, mycountof(tBuf), _T("OnCopyData: channel %d, ptz cmd %d curChannel %d, ucMouseStatus %d "),
			(int)pPtz->iChannel, (int)pPtz->ucCmd, curChannel, pPtz->ucMouseStatus);

		showInfo_open0(0, _T("RealPlayAnd"), tBuf);

		if (CONST_ptzCmd_3ddw == pPtz->ucCmd) {
			Exec3dLocate(pPtz, curChannel);				
			break;
		}

		//
		UINT wmMsg = 0;
		switch (pPtz->ucMouseStatus) {
				case  CONST_mouseStatus_lBtnDown: {
					  wmMsg = WM_LBUTTONDOWN;
					  }
					  break;
				case  CONST_mouseStatus_lBtnUp: {
					  wmMsg  =  WM_LBUTTONUP;
					  }
					  break;		
				default:			
		...
		if (!wmMsg) {
			showInfo_open0(0, 0, _T("OnCopyData: wmMsg err"));
			break;
		}
		...
		switch  (  pPtz->ucCmd  )  {
				case  CONST_ptzCmd_left: {			
					  //
					  this->m_ptz_left.SendMessage(wmMsg);			
					  //		
					  }						
					  //						
					  break;
				case  CONST_ptzCmd_right:
					  this->m_ptz_right.SendMessage(wmMsg);
					  break;
				case  CONST_ptzCmd_up:
					  this->m_ptz_up.SendMessage(wmMsg);
					  break;
				case  CONST_ptzCmd_down:
					  this->m_ptz_down.SendMessage(wmMsg);
					  break;
				case  CONST_ptzCmd_zoomIn:
					  this->m_zoom_tele.SendMessage(wmMsg);
					  break;
				case  CONST_ptzCmd_zoomOut:
					  this->m_zoom_wide.SendMessage(wmMsg);
					  break;
				case  CONST_ptzCmd_focusIn:
					  this->m_focus_near.SendMessage(wmMsg);
					  break;
				case  CONST_ptzCmd_focusOut:
					  this->m_focus_far.SendMessage(wmMsg);
					  break;
				case  CONST_ptzCmd_irisIn:
					  this->m_iris_open.SendMessage(wmMsg);
					  break;
				case  CONST_ptzCmd_irisOut:
					  this->m_iris_close.SendMessage(wmMsg);
					  break;
