...
BOOL CRealPlayAndPTZControlApp::InitInstance()
{
	...
	do {
		...
		if (initShowInfo_func(0, _T("syncQ_showInfo_dh"), errHintWndCls, false, true, (char*)"127.0.0.1", bCli, &gBuf_dhPtz.m_var.pShowInfoStruct)) {
			//goto  errLabel;
			break;
		}
		...
		parseCmdLine_ipcProc(cmdLine, &gBuf_dhPtz.m_var.appParams);
		...
		{
			//
			if (gBuf_dhPtz.m_var.appParams.nvrIp[0] == 0) {
				showInfo_open0(0, 0, _T("dhPtz: nvrIp is null, failed"));
				break;
			}

			//
			gBuf_dhPtz.m_var.m_bAutoClose_ifToolLongNoRecvd_qm = true;
		...
		//
			...
			//
			gBuf_dhPtz.m_var.pQisPipe = qisPipeNew();
			if (!gBuf_dhPtz.m_var.pQisPipe)  break;
			...
			param.pf_onRead = dhPtz_qisPipe_onRead;
			//
			param.uiMaxToInMs_read = CONST_toInMs_pipe_read;

			//
			initQisPipe(&qCfg, pipeName, false, _T("dhPtz"), &param, gBuf_dhPtz.m_var.pQisPipe);
	...
	//
	exitQisPipe(gBuf_dhPtz.m_var.pQisPipe);

	//
	exitShowInfo(&gBuf_dhPtz.m_var.pShowInfoStruct);
