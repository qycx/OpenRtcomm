...
int  jdzy_ca_getRandom()
{
	...
	printLogChar("jdzy_ca_getRandom");
    ...
    //
	{
			...
			if (dwLen > 0)
			{
				...
				if (pRndResponeInfo != NULL)
				{
					...
					if (dwError == AUTH_CODE_SUCCESS)
					{
						...
						printLogChar("strRndReponse ---------begin------");
						printLogChar(strRndReponse.c_str());
						printLogChar("strRndReponse ---------end------");

					}
					else {
						printLogChar("jdzy_ca_getRandom - auth failed ------begin------");
						printLog(pRndResponeInfo);
						printLogChar("jdzy_ca_getRandom - auth failed -------end-------");
					}
		...
		if (dwError != AUTH_CODE_SUCCESS)
		{
			...
			printLogChar("jdzy_ca_getRandom - auth failed");
		}
		...
		if (strlen(resp) != strRndReponse.length()) {
			printLogChar("jdzy_ca_getRandom - length error");
			goto  errLabel;
		}
		...
		printLogChar("jdzy_ca_getRandom - jdzy: getRandom ok");
...
#define			CONST_attName_usrName	L"dnname"
				if (!_tcsicmp(cAttName, CONST_attName_usrName)) {
					...
					for (auto item : splitStr) {
						...
						if (itemStr.size() == 2) {
							showInfo_open0(0, 0, itemStr[0]);
							showInfo_open0(0, 0, itemStr[1]);
						}
...
int  jdzy_ca_login_auth()
{
	...
	printLogChar("jdzy_ca_login_auth");
	...
	if (dwError != AUTH_CODE_SUCCESS) {
		...
		showInfo_open0(0, 0, clientIp);
		showInfo_open0(0, 0, _bstr_t((LPCSTR)pchrecvBuf));

		showInfo_open0(0, 0, _T("auth failed"));
		printLogChar("jdzy_ca_login_auth - auth failed ------begin-------");
		printLogChar(pchrecvBuf);
		printLogChar((char*)clientIp);
		printLogChar("jdzy_ca_login_auth - auth failed -----end------");
		goto  errLabel;
	}
	...
	if (!tUsrName[0]) {
		printLogChar("jdzy_ca_login_auth - tUsrName null");
		goto  errLabel;
	}
	...
	printLogChar("jdzy_ca_login_auth end");
