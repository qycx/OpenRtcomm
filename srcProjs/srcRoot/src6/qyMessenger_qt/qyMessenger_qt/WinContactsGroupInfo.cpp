#include "WinContactsGroupInfo.h"

WinContactsGroupInfo* winContactsGroupInfo = nullptr;
WinContactsGroupInfo::WinContactsGroupInfo(QWidget* parent)
	: QWidget(parent), ui(new Ui::WinContactsGroupInfo)
{
	ui->setupUi(this);
	winContactsGroupInfo = this;
	this->setObjectName("ContactsGroupInfo");
	initControl();
}

WinContactsGroupInfo::~WinContactsGroupInfo()
{
	delete ui;
	if (grpWidget)
	{
...
//显示群组信息
void WinContactsGroupInfo::ShowContactsGroupInfo(WinObjUser user)
{
	if (user.dataType != 3)
	{
		if (user.name == "null")
...
void WinContactsGroupInfo::initControl()
{
	...
	QFile file(":/Resources/QSS/WinContactsGroupInfo.css");
...
void WinContactsGroupInfo::on_btnSendMsg_clicked()
{
	emit toSendMsg(m_curUser);
}

void WinContactsGroupInfo::on_benSendMeeting_clicked()
{
	emit toSendMeeting(m_curUser);
}

void WinContactsGroupInfo::showGroupContactList(CMyDb* pDb, IM_GRP_MEM* pQMem, int64_t* idinfo)
{
	...
	if (pQMem->iStatus == CONST_qyStatus_ok)
	{
		if (pQMem->idInfo_grp.ui64Id == (*idinfo))
		{
			...
			WinContactItem* pContactItem = new WinContactItem(ui->centerWidget);
			pContactItem->setHeadPixmap(QPixmap(":/Resources/Images/WinMain/head.png"));
			pContactItem->setUserName(pSyr);
			pContactItem->setSignName(pDw + " " + pBm);
			ui->gridLayout->addWidget(pContactItem, rows / 4, rows % 4, Qt::AlignLeft);
		}
	}
}
...
int  getGroupByIdInfo(void* p0, void* p1, void* p2)
{
	...
	if (pQMem->iStatus == CONST_qyStatus_ok)
	{
		winContactsGroupInfo->showGroupContactList(pDb, pQMem, idinfo);
	}
	iRet = 0;
errLabel:
	return  iRet;
...
int WinContactsGroupInfo::toGetGroupByIdInfo(int64_t idinfo, QString name)
{
	int  iRet = -1;
	QY_MC* pQyMc = QY_GET_GBUF();
	...
	ui->labelGroupInfo->setText(QString("%1(%2)").arg(name).arg(pDb->m_var.pQ_qyImGrpMemTab->uiQNodes));
	iRet = 0;
errLabel:
	return  iRet;
}
