...
__declspec(dllexport)  int  viewDlgOfflineRes_talk(HWND  hParent, HWND  hCurTalk, BOOL  bNeedNotShowWnd, BOOL  bActivateWnd, HWND* phTool)
{
	...
	QWidget* pParent; pParent = QWidget::find((WId)hParent);
	...
	//
	HWND  m_hWnd; m_hWnd = (HWND)pDlg->winId();
	//
...
int CloudDiskWidget::init()
{
		...
		HWND  m_hWnd; m_hWnd = (HWND)this->winId();

		//
		//cusDlgRes(0, &pQyMc->cusRes, m_hWnd, IDD);

		DLG_guiData_offlineRes* p; p = &m_var.guiData;
		...
		//
		HWND  hParent; hParent = m_hWnd;


		//
		unsigned  int  uiTranNo; uiTranNo = 0;
		...
		//
		TCHAR* hint;hint = (TCHAR*)_T("Please waiting...");
		hint = (TCHAR*)_T("请等待...");
		//
		...
		//
		TCHAR  tBuf[128];
		_sntprintf(tBuf, mycountof(tBuf), (TCHAR*)_T("CloudDiskWidget:init: resServ: %I64u"), pMisCnt->procOfflineRes.idInfo_resServ.ui64Id);
		showInfo_open0(0, 0, tBuf);
...
BOOL  CloudDiskWidget::bRefresh()
{
	...
	//
	HWND  hTalker_res; hTalker_res = NULL;
	...
	//		
	unsigned  int  uiTranNo_wait; uiTranNo_wait = 0;
	{
	...
	hint = (TCHAR*)_T("请等待...");
	//
	int  nStep_finished; nStep_finished = 5;  //  30;
...
BOOL  CloudDiskWidget::bDisplay()
{
	...
	getTalkerDesc(m_var.idInfo, &uiObjType, grpName_peer, mycountof(grpName_peer), mynull, mynull, 0, mynull, 0, srcName_peer, mycountof(srcName_peer));
...
bool CloudDiskWidget::nativeEvent(const QByteArray& eventType, void* message, qintptr* result)
{
	Q_UNUSED(eventType);
...
bool CloudDiskWidget::postMessageQt(MSG* message, qintptr* result)
{
	//
...
void CloudDiskWidget::OnBnClickedButtondelete()
{
	...
	lvItem.pszText = tBuf;
	lvItem.cchTextMax = mycountof(tBuf);
	int iSelectItem; iSelectItem = table_row;
...
void CloudDiskWidget::OnBnClickedButtondownload()
{
	...
	RES_inProcess* pMem; pMem = &m_var.reses_retrieved.mems[iSelectItem];

	//
	HWND  hTalker_res; hTalker_res = NULL;
	BOOL  bNeedNotShowWnd; bNeedNotShowWnd = TRUE;
#ifdef  __DEBUG__
...
__declspec(dllexport)  void  dlgOfflineRes_upload(HWND  hParent, HWND  hDlg_offlineRes, LPCTSTR  fileName)
{
	...
	//
	CloudDiskWidget* pDlg; pDlg = (CloudDiskWidget*)QWidget::find((WId)hDlg_offlineRes);
	if (!pDlg)  goto  errLabel;

	{
	HWND  m_hWnd; m_hWnd = hDlg_offlineRes;
...
void CloudDiskWidget::slot_button_file()
{
    ...
    else if (sender() == ui.pushButton_7)
    {
		...
		if (!pProcInfo->getDirAndFinalName(fileName, tDir, mycountof(tDir), mynull, 0)) {
			saveSelectDir(tDir);
		}
