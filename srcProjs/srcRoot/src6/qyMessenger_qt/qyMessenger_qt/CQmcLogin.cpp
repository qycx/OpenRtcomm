...
//
bool QString2Utf8(const QString& source, char* dest, size_t destSize) {
	if (!dest || destSize == 0) {
		return false;
	}

	QByteArray byteArray = source.toUtf8();
	size_t bytesToCopy = qMin(destSize - 1, static_cast<size_t>(byteArray.size()));

	strncpy(dest, byteArray.constData(), bytesToCopy);
	dest[bytesToCopy] = '\0'; // 确保null终止

	return bytesToCopy == byteArray.size(); // 返回是否完整拷贝
}

bool QString2TChar(const QString& source, TCHAR* dest, size_t destSize) {
	if (!dest || destSize == 0) {
		return false;
	}
	...
	if (length < destSize) {
		source.toWCharArray(dest);
		dest[length] = _T('\0');  // 添加终止符
		//
		bRet = true;
	}
...
void CQmcLogin::onLoginOkclicked(bool)
{
	...
	//
	QString2Utf8(serviceNameLogin, servIp, mycountof(servIp));
 	TCHAR  tUsrName[128];  tUsrName[0] = 0;
	TCHAR  tPwd[128]; tPwd[0] = 0;
	QString2TChar(UserName, tUsrName, mycountof(tUsrName));
	QString2TChar(Password, tPwd, mycountof(tPwd));
	...
	//
	if (0 != doMcClientLogonOK(this, servIp,  tUsrName, tPwd)) {
		...
		//
		qmcLogForHg(0, (wchar_t*)serverTip.utf16(), false);

		return;
	}
	QString log_txt = u8"登录成功";
...
void CQmcLogin::initControl()
{
	...
	if (ui->checkBox_savePasswd->isChecked()) {
		if (qyGetRegCfgT(hKeyRoot0, tQnmSchedulerBuf, _T(CONST_regValName_passwd), (char*)tBuf, sizeof(tBuf), mynull))tBuf[0] = 0;
