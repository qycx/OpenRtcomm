...
void WinSystemSetup::initControl()
{
	...
	ui->BtnOther->setText(u8"其他");
	connect(ui->BtnOther, SIGNAL(clicked()), this, SLOT(on_btnOther_click()));
	...
	WinSystemOther* systemOther = new WinSystemOther();
	ui->SystemstackedWidget->addWidget(systemOther);
	//painterMenu("BtnOther");
...
void WinSystemSetup::changeEvent(QEvent* event) {
	...
	QWidget* widget = ui->SystemstackedWidget->currentWidget();
	if (widget && widget->inherits("WinSystemOther")) {
		((WinSystemOther*)widget)->reload();
	}

	QWidget* WinSystemRule_widget = ui->SystemstackedWidget->currentWidget();
	if (WinSystemRule_widget && WinSystemRule_widget->inherits("WinSystemRule")) {
		((WinSystemRule*)WinSystemRule_widget)->reload();
	}
...
void WinSystemSetup::on_btnOther_click()
{

	painterMenu("BtnOther");
	ui->SystemstackedWidget->setCurrentIndex(4);
	WinSystemOther* systemOther = (WinSystemOther*)ui->SystemstackedWidget->widget(4);
	if (systemOther)
		systemOther->reload();
	//	QWidget* weig = new QWidget();
	//	QLabel* lab = new QLabel(weig);
	...
	//	ui->SystemstackedWidget->addWidget(weig);
		//调用网页
	//ui->SystemstackedWidget->currentWidget()
...
void WinSystemSetup::painterMenu(const QString pushBtnName)
{
	...
	if (pushBtnName.contains("BtnOther"))
	{
		ui->BtnOther->setStyleSheet("color:rgb(22, 154, 218);font-weight:500;");
	}
	else
	{
		ui->BtnOther->setStyleSheet("color:#000;font-weight:none;");
