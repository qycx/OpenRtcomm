...
bool WinBasic::nativeEvent(const QByteArray& eventType, void* message, qintptr* result)
{
    if (!canResize())
    {
        return false;
...
void WinBasic::drawShadow(QPainter* painter)
{
    ...
    for (int i = 0; i < x_nBorder; ++i)
    {
        ...
        //
        //traceLog((TCHAR*)_T("before\n"));
        //
        int iVal = 100 - qSqrt(i) * 80;
        if (iVal < 0)  iVal = 0;
        if (iVal > 255) iVal = 255;
        //
        color.setAlpha(iVal);
        //
        //traceLog((TCHAR*)_T("after\n"));
