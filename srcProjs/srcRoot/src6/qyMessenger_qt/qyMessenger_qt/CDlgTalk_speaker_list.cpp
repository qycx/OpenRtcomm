...
void CDlgTalk_speaker_list::setContent(QString peerId) 
{
	...
	CDlgTalk_qt* pTalk; pTalk = (CDlgTalk_qt*)CDlgTalk_qt::find((WId)hTalk);
	...
	QMC_TASK_INFO* pTaskInfo; pTaskInfo = (QMC_TASK_INFO*)getQmcTaskInfoByIndex(pProcInfo, pm_var->av.taskInfo.iIndex_taskInfo);
	...
	//
	{
		...
		TCHAR bm[128];
		TCHAR syr[128];
		getTalkerDesc(pm_var->av.taskInfo.idInfo_initiator, mynull, mynull, 0, mynull, dw, mycountof(dw), bm, mycountof(bm), syr, mycountof(syr));
