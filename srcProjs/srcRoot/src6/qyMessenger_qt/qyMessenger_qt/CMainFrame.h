...
// 定义事件 ID
#define QY_EVENT_NOTIFY_MSG  (QEvent::Type)(QEvent::User + 1001)

class NotifyMsgEvent : public QEvent
{
public:
	NotifyMsgEvent(SendData _sd, SessionInfo _si, WinContactItem* _pItem)
		: QEvent(QY_EVENT_NOTIFY_MSG), sd(_sd), si(_si), pContactItem(_pItem)
	{
	}
	...
	SendData sd;
	SessionInfo si;
	WinContactItem* pContactItem; //确保这个指针在处理时还未被销毁
};
...
extern CMainFrame* g_pMainWndInstance;
