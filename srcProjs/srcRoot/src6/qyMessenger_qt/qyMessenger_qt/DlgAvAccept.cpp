...
int viewDlgAvAccept()
{
	...
	if (IsWindow(var.notifyTaskStatus.hTool_dlgAvAccept)) {
		...
		iErr = 0;  return iErr;// goto  errLabel;
	}
	int d_WIDTH = 348;
	int d_HEIGHT = 164;
	//
...
int  closeDlgAvAccept()
{
	...
	if (IsWindow(var.notifyTaskStatus.hTool_dlgAvAccept)) {
		...
		//
		var.notifyTaskStatus.hTool_dlgAvAccept = mynull;
	}
	...
	: QWidget(parent)
{
	...
	//
	this->m_pMediaPlaylist = new CustomPlaylist;// QMediaPlaylist
	this->m_pMediaPlaylist->setMediaPlayer(this->m_pMediaPlayer);
	...
	if (!pProcInfo->status.avStatus.bExists_meeting) {
		if (bFileExists((TCHAR*)musicFile.utf16())) {
			...
			if (bSoundCardExists()) {
				...
				m_pMediaPlaylist->addMedia(QUrl::fromLocalFile(musicFile));// "C:/tttbbb/124/recieve.mp3"));
				//
				//this->m_pMediaPlayer->setMedia(0, music);
				//this->m_pMediaPlayer->setPlaylist(this->m_pMediaPlaylist);
				//
				this->m_pMediaPlaylist->setPlaybackMode(SimpleMediaPlaylist::CurrentItemInLoop);
				//
#ifndef  __DEBUG__
	...
	if  (  0 == getAvCallerInfo(&idInfo_logicalPeer, &idInfo_from)  )  {
		if (idInfo_logicalPeer.ui64Id == idInfo_from.ui64Id) {
			...
			getTalkerDesc( idInfo_from, talkerDesc_from, mycountof(talkerDesc_from), mynull, 0);
			...
			getTalkerDesc( idInfo_logicalPeer, &uiObjType,grpName,mycountof(grpName),&idInfo_creator,mynull,0,mynull,0,mynull,0);
...
bool DlgAvAccept::nativeEvent(const QByteArray& eventType, void* message, qintptr* result)
{
	Q_UNUSED(eventType);
...
bool DlgAvAccept::postMessageQt(MSG* message, qintptr* result)
{
	//
