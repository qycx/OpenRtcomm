    ...
    , ui(new Ui::CMainFrame)
{
   ...
   // ui->infoInfo->installEvenFilter(this);
    ...
    WriteLog("init CmainFrame success");
    //
#if 10
...
void CMainFrame::Init()
{
   ...
   // WinTalkList* winTalkList = null;
    ...
    if (pWnd == mynull)  goto  errLabel;
    if (pWnd->objectName() != "WinTalkList")goto  errLabel;
    WinTalkList* pTalkList; pTalkList = (WinTalkList*)pWnd;
    if (pTalkList == mynull)  goto  errLabel;
...
{
    if (event->type() == QY_EVENT_NOTIFY_MSG) {

        NotifyMsgEvent* pNe = static_cast<NotifyMsgEvent*>(event);
        SendData sd = pNe->sd;
        SessionInfo si = pNe->si;
        WinContactItem* pContactItem = pNe->pContactItem;

        CCtxQyMc* pQyMc = g_pQyMc;
        if (!pQyMc) { event->accept(); return; }

        CCtxQmc* pProcInfo = (CCtxQmc*)pQyMc->get_pProcInfo();
        MIS_CNT* pMisCnt = pProcInfo->getMisCntByName(_T(""));

        //获取当前窗口状态
        bool isMainWndVisible = this->isVisible();
        bool isMainMini = this->isMinimized();
        bool isMsgSel = this->isMsgSel();

        QY_MESSENGER_ID curIdInfo;
        this->getCurIdInfo(&curIdInfo);

        //判断是否是自己发的消息
        if (sd.from_id.ui64Id != pMisCnt->idInfo.ui64Id) {

            this->playReciveSound();

            this->flashTaskWindow();
            // }
