...
__declspec(dllexport )  int  parseHgCmd_servReq(void* p0, short  sCmd, char* dataBuf, int  dataBufLen, Param_hgCmd_servReq* pParam)
{
	...
	for (;; ) {
		...
		if (objType > 0 && len > 0) {
			...
			case  CONST_hgType_meetingRecord:
				pParam->hgInfo.bRecord_conf = atol(pStart);
				break;
			...
			break;
			case  CONST_hgType_compereMem:
			{
				QY_MESSENGER_ID	tmp;
				idStr2Info(pStart, &tmp);
				if (pParam->tmpInternal.cnt_compereMems >= mycountof(pParam->hgInfo.compereMems)) {
					showInfo_open0(0, 0, _T("parseHgCmd_servReq failed, too big cnt_compereMems"));
					goto  errLabel;
				}
				pParam->hgInfo.compereMems[pParam->tmpInternal.cnt_compereMems].idInfo.ui64Id = tmp.ui64Id;
				pParam->tmpInternal.cnt_compereMems++;
			}
			break;
			...
			case  CONST_hgType_bLive:
				...
				break;
			case  CONST_hgType_AudioCompressors:
				pParam->conf_ucAudioCompressors = atol(pStart);
				break;
			...
			case  CONST_hgType_usFps:
				pParam->conf_usFps = atol(pStart);
				break;

			case  CONST_hgType_ucb100k:
				pParam->conf_ucb100k = atol(pStart);
				break;

			case  CONST_hgType_confType:
				pParam->usConfType = atol(pStart);
				break;
	...
	if (sCmd == CONST_hgCmd_startMeeting) {
		...
		if (pParam->usOp == 0)
			_sntprintf(tBuf, mycountof(tBuf), _T("parseHgCmd_servReq: startMeeting,ConfType=%d,usFps=%d,ucb100k=%d,usOp=%d,Compressors=%d,bRecord_conf=%d"),
				(int)pParam->usConfType, (int)pParam->conf_usFps, (int)pParam->conf_ucb100k, (int)pParam->usOp, (int)pParam->conf_ucAudioCompressors, pParam->hgInfo.bRecord_conf);
		else
			_sntprintf(tBuf, mycountof(tBuf), _T("parseHgCmd_servReq: stopMeeting,usOp=%d"), (int)pParam->usOp);
			

		showInfo_open0(0, _T("hgFunc"), tBuf);
	}
...
__declspec(dllexport)  int  parseHgCmd_servReqX(void* p0, short  sCmd, char* dataBuf, int  dataBufLen, Param_hgCmd_servReqX* pParam)
{
	...
	for (;; ) {
		...
		//
		char* tag_cmd = (char*)"cmd=";
		int  iCmd = geti64Val(seg, tag_cmd);
		if (iCmd) {
			pParam->iCmd = iCmd;
		}
...
#ifdef  __DEBUG__
		//
		if (objType == CONST_hgType_curConf) {
			int  ii = 0;
		}
#endif 
		...
		if (objType > 0 && len > 0) {
			...
			case CONST_hgType_curConf: {
				int  ii = 0;
				//
				pParam->curConf_ui64Id = atol(pStart);
				//			
				}
				break;
	...
	if (sCmd == CONST_hgCmd_startMeeting) {
		...
		if (pParam->usOp == 0)
			_sntprintf(tBuf, mycountof(tBuf), _T("parseHgCmd_servReq: get cmd startMeeting,usConfType=%d,conf_usFps=%d,conf_ucb100k=%d,usOp=%d"),
				(int)pParam->usConfType, (int)pParam->conf_usFps, (int)pParam->conf_ucb100k, (int)pParam->usOp);
		else
			_sntprintf(tBuf, mycountof(tBuf), _T("parseHgCmd_servReq: get cmd stopMeeting,usOp=%d"), (int)pParam->usOp);
...
{
	unsigned  short usAvLevel = CONST_policyAvLevel_480p;

	//
	switch  (  hg_iHz  ) {
			case  CONST_iHz_240p:
				  usAvLevel = CONST_policyAvLevel_240p;		
				  break;	
			case  CONST_iHz_480p:		
				  usAvLevel = CONST_policyAvLevel_480p;		
				  break;	
			case  CONST_iHz_720p:		
				  usAvLevel = CONST_policyAvLevel_720p;		
				  break;	
			case  CONST_iHz_1080p:		
				  usAvLevel = CONST_policyAvLevel_1080p;		
				  break;	
			default:		
					break;
	}
