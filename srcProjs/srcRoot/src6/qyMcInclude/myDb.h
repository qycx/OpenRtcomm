...
public:
	...
	struct					{
		...
		//
		GENERIC_Q		*	pQ_qyImObjTab;
		GENERIC_Q		*	pQ_qyImObjRegInfoTab;
		GENERIC_Q		*	pQ_qyImObjRegInfoTab1;
		GENERIC_Q		*	pQ_qyImGrpInfoTab;
		GENERIC_Q		*	pQ_qyImGrpMemTab;
		GENERIC_Q		*	pQ_qyImObjRuleTab;
		GENERIC_Q		*	pQ_qyImTaskTab;
		GENERIC_Q		*	pQ_qyImMsgTab;
		GENERIC_Q		*	pQ_qyDynBmpTab;

	}						m_var;

public:
	BOOL  bQyOpenDb(  LPCWSTR  connectStr  );
	void  qyCloseDb(  );
...
//
typedef  struct  __qMem_qyImObj_t		{
				 QY_MESSENGER_INFO		messengerInfo;
				 TCHAR					desc[64];
}		 QMEM_qyImObj;

typedef  struct  __qMem_qyImObjRegInfo_t								{
				 struct													{
					 unsigned  int										uiType;
					 //
					 int												id;
...
//  2013/02/10
typedef  struct  __qMem_qyImObjRule_t	{
				 QY_imObjRule_rcd		ruleRcd;
}		 QMEM_qyImObjRule;

typedef  struct  __qMem_qyDynBmp_t		{
		 QY_dynBmp_RCD					dynBmpRcd;
}		 QMEM_qyDynBmp;
//
typedef  BOOL  (  *PF_bGetImAmRcdBySth  )(  void  *  pDbParam,  int  iDbType,  LPCTSTR  misServName,  QY_MESSENGER_ID  *  pIdInfo,  unsigned  short  usAuthType,  LPCTSTR  messengerName,  int  id,  QY_imAm_RCD  *  pObj  );
...
typedef  BOOL  (  *PF_bViewImGrpList  )(   QY_MC  *  pQyMc,  QY_OBJ_DB  *  pObjDb,  LPCTSTR  docName,  LPCTSTR  wherePart  );
typedef  BOOL  (  *PF_bViewImGrpMemList_isClient  )(  QY_MC  *  pQyMc,  QY_OBJ_DB  *  pObjDb,  LPCTSTR  docName,  LPCTSTR  wherePart  );

//
typedef  BOOL  (  *PF_bViewImObjRuleList_is  )(  QY_MC  *  pQyMc,  QY_OBJ_DB  *  pObjDb  );
...
//
struct  __qmDbFuncs_t	{

				 PF_bGetImAmRcdBySth						pf_bGetImAmRcdBySth;
