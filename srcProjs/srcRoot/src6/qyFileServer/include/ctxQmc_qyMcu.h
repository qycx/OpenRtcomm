...
//
class  CCtxQmc_qyMcu: public  CCtxQmc
{
public:
	CCtxQmc_qyMcu(  );
	virtual  ~CCtxQmc_qyMcu(  );
	...
	//typedef  struct  __mcVar_isCli_t								{
				 ...
				 struct		{
					 ...
					 struct									{
						 struct								{
							 int							nTimes_toWaitForConnection;
						 }									status;
...
//}		 MC_VAR_isCli;
		...
		//
		virtual int  stopMosaicThreads(  QMC_mosaic_maker  *  pMosaic  );
		virtual int  setFlg_inConfMosaic(  HWND  hDlg,  int  iWndContentType,  int  iIndex_sharedObj,  int  iIndex_sharedObjUsr  );
		virtual int  makeMosaicStream(  HWND  hDlg,  int  iTaskId,  int  level,  AV_COMPRESSOR_CFG  *  pCompressor_conf,  AV_COMPRESSOR_CFG  *  pCompressor_resource,  int  iFourcc_resource,  QMC_mosaic_maker  *  pMosaic  );
		virtual int  openLocalVideo_mosaic(  MC_VAR_common  *  pProcInfoCommon,  unsigned  int  uiCamCapType,  BITMAPINFOHEADER  *  pBih_photomosaic,  VIDEO_COMPRESSOR_CFG  *  pVideoCompressorParam,  int  iIndex_sharedObj  );


		virtual  int  do_addToRecentMsg(HWND  hDlgTalk, void* pDLG_TALK_var, long  lRowIndex, int  iTaskId, QY_MESSENGER_ID* pIdInfo_talker, LPCTSTR  talkerDesc, unsigned  short  usOp, int  iStatus, char* timeBuf, LPCTSTR  rowIdStr, LPCTSTR  content, BOOL  bFollowingRows, BOOL  bScrollIntoView, TCHAR* txtBuf, unsigned  int  uiTxtBufCnt)
		{
			return -1;
		}
		...
		virtual int do_test(HWND  hWnd) { return -1; }
		...
		//
		virtual int reportToHg_meetingOn(unsigned  __int64  uiMeetingId_hg, int  iMeetingType, unsigned  __int64  ui64Id_grp,  bool  bOn)
		{
			return  -1;
		}

		virtual int reportToHg_saveAv(unsigned  __int64  uiMeetingId_hg,  char* video_url, char* resource_url) {
			return  ::reportToHg_saveAv(uiMeetingId_hg,video_url, resource_url);
		}
		...
		{
			return  ::myGetMbFlg(iMeetingLevel);
		}
		...
		virtual int  sendStatusConfLayoutParam(HWND  hDlgTalk, QY_MESSENGER_ID  idInfo_to, LPCTSTR  hint);

		//
		virtual BOOL bSupported_emergencyCommand() { 
			return  false; 
		}
...
//
#define		CONST_intervalInS_mcuXt			20
...
//
int  doConf_showDlgMcClientLogon(  HINSTANCE  g_hInst,  HWND  hMainWnd,  void  *  pQyMc,  PARAM_showDlgMcLogon  * pParam  );
