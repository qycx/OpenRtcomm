...
/***** Pelco_D : Command2 *****/
enum  {
	  PELCO_D_C2_FOCUS_FAR			=	0x80;		//  焦距远
	  PELCO_D_C2_ZOOM_WIDE			=	0x40;		//  变倍大
	  PELCO_D_C2_ZOOM_TELE			=	0x20;		//  变倍小
	  PELCO_D_C2_DOWN				=	0x10;
	  PELCO_D_C2_UP					=	0x08;
	  PELCO_D_C2_LEFT				=	0x04
	  PELCO_D_C2_RIGHT				=	0x02;
};
...
namespace  Pelco_D_Protocol  {
	...
	class  CPelco_D  {
		public:
			...
			BYTE  *  BackLightCompensation(  UINT  deviceAddress,  Switch  action  );
			BYTE  *  AutoWhiteBalance(  UINT  deviceAddress,  Switch  action  );
			BYTE  *  EnableDevicePhaseDelay(  UINT  deviceAddress  );
