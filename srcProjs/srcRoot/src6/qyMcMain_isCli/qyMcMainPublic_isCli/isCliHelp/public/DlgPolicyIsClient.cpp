 ...
 BOOL  CDlgPolicyIsClient::bShowPolicy(  )
{
	...
	SetDlgItemText(IDC_EDIT_dirToSaveICFile, m_var.policy.dirToSaveICFile);
...
BEGIN_MESSAGE_MAP(CDlgPolicyIsClient, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_browseIcDir, OnBnClickedButtonbrowseICdir)
...
void CDlgPolicyIsClient::OnOK()
{
	...
	GetDlgItemText(IDC_EDIT_dirToSaveICFile, m_var.policy.dirToSaveICFile, mycountof(m_var.policy.dirToSaveICFile));
	tTrim(m_var.policy.dirToSaveICFile);
	if (m_var.policy.dirToSaveICFile[0]) {
		if (!bDir(m_var.policy.dirToSaveICFile)) {
			qyShowHint(_T("The directory to save ic file does not exist"));  return;
		}
	}
	if (lstrcmpi(m_var.policy.dirToSaveICFile, m_var.pProcInfo->cfg.policy.dirToSaveICFile)) {
		if (!ucbAsked && myMessageBox(m_hWnd, getResStr(0, &pQyMc->cusRes, CONST_resId_whetherToModify), _T(""), MB_YESNO) != IDYES)  return;
		ucbAsked = TRUE;
		qySetRegCfgT(reg.hKeyRoot0, reg.rootKey, _T(CONST_regValName_dirToSaveICFile), m_var.policy.dirToSaveICFile);
	}
...
void CDlgPolicyIsClient::OnBnClickedButtonbrowseICdir()
{
	// TODO: ÔÚ´ËÌí¼Ó¿Ø¼þÍ¨Öª´¦Àí³ÌÐò´úÂë
	CString		path;
	CQyString	dirStr;
	QY_MC* pQyMc = QY_GET_GBUF();
	int			idc = IDC_EDIT_dirToSaveICFile;
