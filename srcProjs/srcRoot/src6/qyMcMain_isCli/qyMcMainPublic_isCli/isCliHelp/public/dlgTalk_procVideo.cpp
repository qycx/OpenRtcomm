...
//
		...
		if (!hCtrl) {		//  没有指定时,都放到peer里去
			if (pMsg->pkt.uiTaskType == CONST_imTaskType_viewDynBmp) {
				...
				else if (m_var.addr.uiObjType == CONST_objType_messenger) {
					hCtrl = m_var.av.hCtrl_RemoteVideo;
					//pCapRect  =  &m_var.guiData.bgWallCapRect;
					//
					hWnd_dst = m_var.av.hWnd_dst_RemoteVideo;//GetDlgItem(hDlg, idc);
					phDc_dst = &m_var.av.hDc_RemoteVideo;

					pZone = &m_var.av.remoteVideoZone;
					//
					pSubWnds = &pDlgMgrVar->subWnds;
				}
				...
				if (pMsg->pkt.uiTaskType == CONST_imTaskType_viewDynBmp) {
					if (pMsg->pkt.decInfo.dynBmp.resObj.uiObjType == CONST_objType_smallStream) {
						...
						if (dwTickCnt - pDlgMgrVar->av.taskInfo.dwLastTickCnt_confStarter_sendReq_toStopSending > 200) {
							...
							idInfo_grp_related.ui64Id = pDlgMgrVar->addr.idInfo.ui64Id;
							//
							MSGR_ADDR  addr;
							...
							procSendTaskProcReq(m_var.pMisCnt, CONST_qyCmd_sendMedia, CONST_imOp_stopSending, 0, 0, 0, idInfo_grp_related,pMsg->iTaskId, 0, &pMsg->pkt.decInfo.dynBmp, CONST_channelType_media, bOutputDirectly, tmppHint, &addr);
						}
