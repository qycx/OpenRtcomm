...
//}		 MC_VAR_isCli;
		...
		virtual  int  do_talk_OnTimer(HWND  hDlgTalk,void *pDLG_TALK_var) = null;
		...
		//
		virtual int  getDirAndFinalName(LPCTSTR  svFileName, TCHAR* dirName, unsigned  int  size, TCHAR* finalName, unsigned  int  finalNameSize) = null;
		...
		virtual void gui_notify_clearTask(int  iTaskId) = null;
		virtual void gui_notify_clear() = null;

		//
		virtual void* DBManager_new()=null;
		virtual void DBManager_free(void ** ppDBManager)=null;
		//
		virtual int initDBManager(void * pDBManager) = null;

		//
		virtual int  do_dlgTalk_procTask_transferAvInfo(HWND  hTalk)=null;

		//
		virtual int AEC_layout_chk(AEC_layout* pLayout)=null;

		//
		virtual int  do_mainWnd_OnTimer(HWND  hMainWnd, void* pVar, UINT  nIDEvent) = null;

		//
		virtual  int  displayRecentFriends(MIS_MSG_displayRecentFriends_qmc* pMsg) = null;

		//
		virtual  int  confKeyChanged(HWND  hDlgTalk) = null;

		//
		virtual int popup_permitToSpeak(HWND  hDlgTalk)=null;

		//
		virtual int do_dlgTalk_proc_recvd_confCtrlState(HWND  hDlgTalk) = null;

		//
#ifdef  __DEBUG__
#endif
		//
		virtual int sxrz_yq(QIS_ca_req* p) = null; 	//  双向认证，客户端验签

		//
		virtual  bool  bNeedUsrLogin_sm() = null;
...
//
int initSavers(bool  bRecord,bool bLive,TCHAR  *  liveUrl, __int64 meetingId,  unsigned  __int64  peerId);
int exitSavers();
...
//
extern  "C"  BOOL  bServConnected_mis(QY_ENV * pEnv, char* serverIp, unsigned  short  serverPort, void* pParam, int  serviceId, char* ver, QY_ENC_CTX * pCommEncCtx, SOCK_TIMEOUT * pTo);
