...
//  2015/10/20
 int  doPostEnc_audio(  void  *  pQDC_OBJ_INFO,  char  *  pInput,  unsigned  int  inputSize,  int  iSampleTimeInMs,  unsigned  int  uiPts,  void  *  pBuf_pkt  )
 {
	...
	TRANSFER_AUDIO_DATA* pReq; pReq = pCapProcInfo->compressAudio.pReqCache;	//  (  TRANSFER_AUDIO_DATA  *  )(  pPkt->memory.m_pBuf  +  pPkt->uiSize_dec  );		
	if  (  !pReq  )  {
		goto  errLabel;
	}
	...
	//
	//  2016/12/10	
	QY_Q2* p_realTimeMediaQ2; p_realTimeMediaQ2 = pProcInfo->get_realTimeMediaQ2();
	...
	int  myuiSampleTimeInMs; myuiSampleTimeInMs = 0;
	unsigned  int  myuiPts; myuiPts = 0;	//  2015/01/15
	//int  usIndex_activeMems_from  =  0;
					   ...
					   DWORD  dwTickCnt;
					   int  i;
					   COMPRESS_AUDIO* pCompressAudio; pCompressAudio = &pCapProcInfo->compressAudio;
					   unsigned  char  ucbNeedShowFrameInfo; ucbNeedShowFrameInfo = FALSE;
...

		 {
			 ...
			 //
#if  1  //def  __DEBUG__
			 MIS_MSGU* pMsgBuf; pMsgBuf = NULL;		//  2016/12/10. 要在压缩初始化时分配，在停止时释放。否则速度有影响。
#endif
