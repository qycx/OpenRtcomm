...
int  getWebcamInfo(  unsigned  int  uiObjType,  int  index_obj,  WEBCAM_info  *  pWebcamInfo  )
{
	...
	switch  (  uiObjType  )  {
			...
			case  CONST_objType_ic:
				pRegKeyName = _T(CONST_regKeyName_ic);
				  break;
			default:
					return  -1;
	}
...
__declspec(  dllexport  )  int  showDlgOpAvDev(  HWND  hParent,  HWND  hDlg_shareDynBmps,  int objType,  int  index_obj_selected,  BOOL  *  pbUnresizable  )
{
	...
	case  CONST_objType_ic:
		break;
	default:
		showInfo_open0(0, 0, _T("showDlgOpAvDev failed: objType err"));
		return  -1;
		break;
	}
...
int  CDlgOpAvDev::refreshCtrlStatus(  )
{
	...
	//
	if  (  dlgShareDynBmps_bShared(  m_var.hDlg_shareDynBmps,  m_var.objType,  m_var.index_obj_selected  )  )  {
		bEnable  =  FALSE;
	}
...
BOOL CDlgOpAvDev::OnInitDialog()
{
	...
	_sntprintf(  tBuf,  mycountof(  tBuf  ),  _T(  "%s %s %d"  ),  getResStr( 0, &pQyMc->cusRes,  CONST_resId_opAvDev  ),  ptObjName,  m_var.index_obj_selected  );
	SetWindowText(  tBuf  );
	
	WEBCAM_info  webcamInfo;
	getWebcamInfo(  m_var.objType,  m_var.index_obj_selected,  &webcamInfo  );
	...
	case  CONST_objType_ic: {
		i = 0;
		safeTcsnCpy(_T("工业相机"), tName, mycountof(tName));
		_sntprintf(tBuf, mycountof(tBuf), _T("%d: %s"), i + 1, tName);
		((CComboBox*)GetDlgItem(idc))->InsertString(-1, tBuf);
		//
...
void CDlgOpAvDev::OnBnClickedButtontoshare()
{
	...
	//
#ifdef  __DEBUG__
	traceLog((TCHAR*)_T("for test: dlgOpADev: bUnresizable set to true"));
	if (m_var.objType == CONST_objType_ic) {
		m_var.bUnresizable = true;		
	}
#endif 
