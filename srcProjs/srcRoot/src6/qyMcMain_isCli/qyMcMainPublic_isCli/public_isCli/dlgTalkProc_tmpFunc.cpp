   ...
   CQyString  getQmInternalUrl(  int  iTaskId,  LPCTSTR  rowIdStr,  int  iOp,  LPCTSTR  paramStr,  LPCTSTR  displayStr  )
{
	...
	CQyString	str;
	str  =  CQyString( (char*) "<a href=\""  )  +  CQyString(  CONST_qmInternalUrl_root  )  +  CQyString(  taskIdStr  )  +  CQyString(  (char*)"/"  )  +  CQyString(  rowIdStr  ?  CQyString(rowIdStr)  :  CQyString(  (char*)""  )  ) + CQyString((char*)"/") + CQyString(opStr) + CQyString((char*)"/") + CQyString(paramStr ? paramStr : _T("")) + CQyString((char*)"/default.htm\">") + CQyString(displayStr) + CQyString((char*)"</a>");
							    ...
							    switch  (  iStatus  )  {
										case  CONST_imTaskStatus_applyToSend:	
											  _sntprintf(  tBuf,  cnt,  _T(  "%s %s, %s: %s, %s"  ),  getResStr(  0,  &pQyMc->cusRes,  CONST_resId_reqToSend  ),
																				...
																				bIncludeUrl  ?  getQmInternalUrl(  pMsg->task.iTaskId,  rowIdStr,  CONST_imOp_send_permit,  0,  CQyString(_T(  "Allow"  ))  )  :  CQyString(_T(  "Allow"  )),
