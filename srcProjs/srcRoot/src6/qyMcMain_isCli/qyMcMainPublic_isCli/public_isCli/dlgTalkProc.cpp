...
errLabel:
 ...
 int  confStarter_sendToCancelTaskAv(  HWND  hDlgTalk,  DLG_TALK_var  &  m_var  )
 {
	 ...
	 int  lenInBytes;
					  ...
					  //  2015/09/07
					  unsigned  int  uiChannelType; uiChannelType = pMsgTask->uiChannelType;
					  //  2015/09/07
					  uiChannelType  =  0;
					  //
  ...
  int  doCmd_startShareMediaFile(  QY_MC  *  pQyMc,  HWND  hDlg,  int  iWndContentType,  int  level,  LPCTSTR  pathCStr,  int  *piIndex_sharedObj,  int  *  piIndex_sharedObjUsr  )
{
	...
	//
#if  10  //  2015/10/11
	BOOL  bAudio; bAudio = TRUE;	//  bNoAudio(  pSharedObj->pShareMediaObj  );
	//
 ...
 int  doCmd_startShareMediaDevice(  QY_MC  *  pQyMc,  HWND  hDlgTalk,  void  *  pDLG_TALK_var,  unsigned  int  uiSubCapType,  int  level,  void  **  ppCapStuff  )
{
	...
	int								iIndex_capBmp;
	CAP_procInfo_mediaDeviceBmp* pCapBmp; pCapBmp = NULL;
	COMPRESS_VIDEO* pCompressVideo; pCompressVideo = NULL;
	int								iIndex_capAudio;
	CAP_procInfo_mediaDeviceAudio* pCapAudio; pCapAudio = NULL;
	COMPRESS_AUDIO* pCompressAudio; pCompressAudio = NULL;
	//
#if  0
	int								iIndex_shmCmd;
...
#endif
	unsigned  int					uiTaskType;
	unsigned  int					uiCapType; uiCapType = CONST_capType_mediaDevice;
	...
	AV_COMPRESSOR_CFG* pCompressor; pCompressor = &mediaDeviceCompressor;
	//
	...
	//
	BOOL	bAudio; bAudio = CAP_STUFF_bAudio(*ppCapStuff);
	BOOL	bVideo; bVideo = CAP_STUFF_bVideo(*ppCapStuff);
 ...
 int  dlgTalk_chkRecvdTasks(  HWND  hDlgTalk,  DLG_TALK_var  &  m_var  )
{
	...
	//					   	
	int					nElapseInS = 20;// MAX_nElapseInS;	//  20;					   
 ...
 int  confStarter_chkActiveMems(  HWND  hDlgTalk,  DLG_TALK_var  &m_var  )
{
	...
	//
	DWORD  dwTickCnt; dwTickCnt = myGetTickCount(NULL);
	MIS_CNT* pMisCnt; pMisCnt = m_var.pMisCnt;

	//
	bool  bChanged; bChanged = false;
...
int  dlgTalk_chkAsAdvertisingPlayer(  HWND  hDlgTalk,  DLG_TALK_var  *  pDlgTalkVar,  BOOL  *  pbNeedClose,  TCHAR  *  tHint,  unsigned  int  uiCnt_tHint  )
{
	...
	//  2014/06/11
	BOOL  bExists_dynBmpRule; bExists_dynBmpRule = FALSE;
	int  i;
	...
	//
	BOOL  bExists_remoteAssist; bExists_remoteAssist = FALSE;
...
int  dlgTalk_chkFileServer(  HWND  hDlgTalk,  DLG_TALK_var  *  pDlgTalkVar,  BOOL  *  pbNeedClose,  TCHAR  *  tHint,  unsigned  int  uiCnt_tHint  )
{
	...
	int  iElapseInMs; iElapseInMs = 0;

	//
	BOOL  bExists_fileTask; bExists_fileTask = FALSE;
	iElapseInMs  =  dwTickCnt  -  pShadowMgr->fileTaskInfo.dwLastModifiedTickCnt_taskStatus;
	int  nIntervalInMs; nIntervalInMs = 9000;
	nIntervalInMs  =  20000;
...
int  tmpHandler_chkAutoAnswer(  void  *  hDlgTalkParam,  void  *  p1,  void  *  pMsgParam  )
{
	...
	TALKER_shadow_mgr* pShadowMgr; pShadowMgr = (TALKER_shadow_mgr*)m_var.pShadowInfo;
	if  (  !pShadowMgr  )  goto  errLabel;
	...
	//
	MIS_MSG_TASK* pMsgTaskElem; pMsgTaskElem = &pMsgElem->task;
	IM_CONTENTU* pContentElem; pContentElem = M_getMsgContent(pMsgTaskElem->ucFlg, &pMsgTaskElem->data);
	if (pContentElem == mynull)goto  errLabel;
	MIS_MSGU* pMsg; pMsg = mynull;
	//
	if  (  pProcInfo->bAppQt()  )  {
...
int  logConfLog(  TASK_respInfo_mem  *  pRespInfoMem,  int  iState,  time_t  tTime_state  )
{
	...
	//	
	CONF_log* pLog; pLog = &pRespInfoMem->var.logs.mems[pRespInfoMem->var.logs.usCnt];
	pLog->iState  =  iState;
...
int  sendConfState(  HWND  hDlgTalk_mgr,  DLG_TALK_var  *  pMgrVar  )
{
	...
	TASK_respInfo* pRespInfo; pRespInfo = get_respInfo(pProcInfo, pMgrVar->av.taskInfo.iIndex_taskInfo);
	if  (  !pRespInfo  )  goto  errLabel;

	//
	//
	CONF_state  cs; memset(&cs, 0, sizeof(cs));// = { 0 };
	cs.uiType  =  CONST_imCommType_confState;
	cs.idInfo_imGrp_related.ui64Id  =  pMgrVar->addr.idInfo.ui64Id;

	//
	BOOL  bSendAll; bSendAll = FALSE;
	...
	//
	unsigned  short  usMaxCnt_perSnd; usMaxCnt_perSnd = MAX_confMemStates;
#ifdef  __DEBUG__
		#if  1	//  for TEST
			 //usMaxCnt_perSnd  =  2;
		#endif
  ...
  VOID CALLBACK dlgTalk_myTimerProc(  HWND hDlgTalk,  UINT uMsg,  UINT_PTR idEvent,  DWORD dwTime  )
{
	...
	//  2009/10/18
	int  loopCtrl_timer_dlgTalk_chkRecvdTasks; loopCtrl_timer_dlgTalk_chkRecvdTasks = pProcInfo->cfg.loopCtrl_timer_dlgTalk_chkRecvdTasks;
	...
	//
	int  loopCtrl_chkAsAdvertisingPlayer; loopCtrl_chkAsAdvertisingPlayer = pProcInfo->cfg.loopCtrl_timer_dlgTalk_chkRecvdTasks;
	...
	//  2017/01/10
	int  nTimes_chkTalkerRules; nTimes_chkTalkerRules = 5;
	int  nTimes_chkDynBmps; nTimes_chkDynBmps = 30;
	...
	//
	_sntprintf(  tBuf,  mycountof(  tBuf  ),  _T(  "%s: %d, %s: %d kbps "  ),  _T(  "Down"  ),  pProcInfo->status.netStat.ins.uiInSpeedInKbps,  _T(  "Up"  ),  pProcInfo->status.netStat.ins.uiOutSpeedInKbps  );
	...
	//  2011/08/01
	#ifndef  __WINCE__
	QM_SHM_CMDS* pShmCmds; pShmCmds = QY_GET_shmCmds(pProcInfo);
			 if  (  pShmCmds  )  {
...
errLabel:
 ...
 int  doSth_afterInitDlgMsgTool(  QY_MC  *  pQyMc,  HWND  hDlgMsgTool,  int  iWndContentType  )
 {
	 ...
	 HWND  hMgr; hMgr = hDlgTalk;
	 if  (  !isTalkerShadowMgr(  pDlgTalkVar->addr  )  )  {
		 TALKER_shadow  *  pShadow  =  (  TALKER_shadow  *  )pDlgTalkVar->pShadowInfo;
		 if  (  !pShadow  )  goto  errLabel;
		 hMgr  =  pShadow->hMgr;
	 }
	 DLG_TALK_var* pMgrVar; pMgrVar = (DLG_TALK_var*)help_getDlgTalkVar_mgr.getVar(hMgr);
	 if  (  !pMgrVar  )  goto  errLabel;
 ...
 int  dlgTalk_OnInitDialog(  HWND  hDlgTalk,  void  *  pDLG_TALK_var  )
{
	...
	//  2015/11/15
	QMC_objVar_info* pTalkerInfo; pTalkerInfo = getQmcObjVarInfoByIndex(pProcInfo, m_var.iIndex_talkerInfo);
	...
	//
	QNM_cusRes_moduleMem* pResMem; pResMem = getCurCusResModuleMem(&pQyMc->cusRes);
	...
	//  2010/09/03
	DWORD	dwStyle; dwStyle = GetWindowLong(hDlgTalk, GWL_STYLE);
	...
	//  2015/08/10
	TALKER_shadow_mgr* pShadowMgr; pShadowMgr = NULL;
	HWND				hMgr; hMgr = NULL;
	{
		CHelp_getDlgTalkVar  help_getDlgTalkVar_mgr;
		hMgr  =  hDlgTalk;
  ...
  int  dlgTalk_cancelTasks(  HWND  hCurTalk,  void  *  pCurDLG_TALK_var,  int  iTaskId,  BOOL  bAutoCancel  )
{
	...
	//  2015/08/04
	BOOL  bNeedClose; bNeedClose = FALSE;
	if  (  pProcInfo->cfg.policy.ucbRunAsAdvertisingPlayer  )  {				
		//
		bNeedClose  =  FALSE;			
...
errLabel:
  ...
  int  getVal_bTaskSlave(  HWND  hDlg,  DLG_TALK_var  &  m_var,  int  iTaskId,  BOOL  *  pbExists  )
  {
	  ...
	  int  iIndex_sharedObj_master; iIndex_sharedObj_master = m_var.av.iIndex_sharedObj_localAv;

	  QY_SHARED_OBJ* pSharedObj_master; pSharedObj_master = NULL;
	  ...
	  SHARE_dyn_bmp* pShareDynBmp_slave; pShareDynBmp_slave = (SHARE_dyn_bmp*)pSharedObj_master->master.pShareDynBmp_slave;
...
int  tmpHandler_showMsg_displayTaskStatus(  HWND  hDlg,  DLG_TALK_var  *  pm_var,  MIS_MSG_TASKSTATUS  *  pMsgTaskStatus,  MIS_MSG_TASK  *  pMsgTask,  IM_CONTENTU  *  pContent,  int  iStatus  )
{
	...
	switch  (  pMsgTaskStatus->iStatus  )  {	
			...
			case  CONST_imTaskStatus_receiving:
				  ...
				  #else
					    _sntprintf(  tBuf,  mycountof(  tBuf  ),  _T(  "%d%%(%I64uk)"  ),  pMsgTaskStatus->u.transferFileStatus.ucPercent,  pMsgTaskStatus->u.transferFileStatus.ui64Len_recvd  /  1024  );						
						addToRecentMsg(  hDlg,  &m_var,  -1,  pMsgTaskStatus->iTaskId,  mynull,  mynull,  pMsgTaskStatus->idInfo_mem,  NULL,  CONST_imOp_tryToSendFile,  pMsgTaskStatus->iStatus,  NULL,  NULL,  tBuf,  0,  0,  0,  0  );								
				  #endif
				  //
				  //
				  break;				
			...
			case  CONST_imTaskStatus_autoCanceledByReceiver:	//  2014/07/30
				  ...
				  if  (  pMsgTaskStatus->iStatus  ==  CONST_imTaskStatus_sendFinished  &&  pMsgTaskStatus->bImGrpMem  )  {	//  ¶ÔÓÚ·Ö×é£¬³ÉÔ±²»ÄÜ¸Ä±äÈÎÎñ×´Ì¬¡£¹ÊÖ±½ÓÏÔÊ¾Ò»¸öÏûÏ¢£¬¼ÓÇ¿Ö±¹ÛÐÔ
					  ...
					  break;
						  ...
						  addToRecentMsg(  hDlg,  &m_var,  -1,  pMsgTaskStatus->iTaskId,  mynull,  mynull,  idInfo_talker,  NULL,  CONST_imOp_tryToSendFile,  pMsgTaskStatus->iStatus,  NULL,  NULL,  tBuf,  0,  0,  0,  0  );																		
...
BOOL  bAutoAccepted_avCall(  HWND  hDlgTalk,  QY_imObj_rules  *  pRules,  unsigned  int  uiTaskType  )
{
	...
	DLG_TALK_var* pMgrVar; pMgrVar = pm_var;

	QY_MC* pQyMc; pQyMc = pMgrVar->pQyMc;// QY_GET_GBUF();
...
int  dlgTalk_proc_recvd_confState(  HWND  hDlgTalk_mgr,  DLG_TALK_var  *  pMgrVar,  MIS_MSG_TASK  *  pMsgTask,  CONF_state  *  pConfState  )
{
	...
	//
	int  tmp_usCnt; tmp_usCnt = min(pRespInfo->usCnt, MAX_taskRespInfoMems);
	//
	BOOL  bNeedRefresh; bNeedRefresh = FALSE;
	//
	int  i;
	for  (  i  =  0;  i  <  pConfState->usCnt;  i  ++  )  {
		 CONF_mem_state  *  pMem  =  &pConfState->mems[i];
...

{
	...
	case  CONST_imCommType_taskInteractionReq: {
		...
		else  if (pContent->taskInteractionReq.usOp == CONST_imOp_pleaseSpeak) {
			
			//
			pProcInfo->do_pleaseSpeak(hMgr, pMsg->task.data.route.idInfo_from);

			//
			break;
			}
		else  if (pContent->taskInteractionReq.usOp == CONST_imOp_pleaseStopSpeaking) {

			//
			pProcInfo->do_pleaseStopSpeaking(hMgr, pMsg->task.data.route.idInfo_from);
	...
	IM_CONTENTU* pContent)
{
	...
	//
	int bAmplification; bAmplification = pContent->confKey.confLayoutParam.respVideoStatus.bAmplification;
	...
	IM_CONTENTU* pContent)
{
	...
	//
	int bAmplification; bAmplification = pContent->statusConfLayoutParam.confLayoutParam.respVideoStatus.bAmplification;

	

	if (pProcInfo->m_iCtxSubtype == CONST_ctxSubtype_qmcQt || pProcInfo->m_iCtxSubtype == CONST_ctxSubtype_qmcSm)
	{
		if (bAmplification) {
			...
			if (pTalkerInfo) {
				DLG_TALK_var* pm_var = (DLG_TALK_var*)pTalkerInfo->var.pData;

				HWND  hTalk = mynull;
				CCtxQyMc* pQyMc = g_pQyMc;
				if (findTalker_shadow(pQyMc, pm_var->addr.idInfo.ui64Id, CONST_talkerSubtype_video, &hTalk) == 0) {
					::PostMessage(hTalk, CONST_qyWm_postVideoAmplifierOk, 0, 0);
				}

				/*
				if (pm_var) {
					pm_var->m_hParent = pm_var->m_hParent;
					::PostMessage(pm_var->m_hSelf, CONST_qyWm_postVideoAmplifierOk, 0, 0);
				}
				*/
			}
		}
			...
			if (pTalkerInfo) {
				...
				CCtxQyMc* pQyMc = g_pQyMc;
				if (findTalker_shadow(pQyMc, pm_var->addr.idInfo.ui64Id, CONST_talkerSubtype_video, &hTalk) == 0) {
					::PostMessage(hTalk, CONST_qyWm_postVideoAmplifierCancel, 0, 0);
				}
 ...
 int  dlgTalk_OnQyPostComm(  HWND  hDlg,  void  *  pDLG_TALK_var,  WPARAM  wParam,  LPARAM  lParam  )
{
	...
	switch  (  wParam  )  {
			...
			case  CONST_qyWmParam_doPost_drawCapImages:		//  2014/04/12
				  ...
				  hWnd_dst = M_GetDlgItem(hDlg, m_var.av.hWnd_dst_RemoteVideo);
				  doPost_drawCapImages(hWnd_dst, m_var.av.hDc_RemoteVideo, lParam, &m_var.av.remoteVideoZone.images);
 ...
 int  dlgTalk_refresh_usLayoutType_cur(  HWND  hDlgTalk,  BOOL  *  pbModified  )
{
	...
	//  2013/02/28
	unsigned  short  usLayoutType_local; usLayoutType_local = dlgTalk_get_usLayoutType_local(hDlgTalk);

	//
	int  nScreens,nMediaFiles; nScreens = 0, nMediaFiles = 0;
	int  nMediaFiles_unresizable; nMediaFiles_unresizable = 0;											//  2015/03/12
	...
	//
	//
	int  nTasks_remoteVideo; nTasks_remoteVideo = 0;
	int  nTasks_unresizable_remoteVideo; nTasks_unresizable_remoteVideo = 0;
	int  iW_unresizable_remoteVideo; iW_unresizable_remoteVideo = 0;
	int  iH_unresizable_remoteVideo; iH_unresizable_remoteVideo = 0;
 ...
 int  openLocalVideo_slave(  MC_VAR_common  *  pProcInfoCommon,  unsigned  int  uiCamCapType,  int  iIndex_sharedObj_master,  VIDEO_COMPRESSOR_CFG  *  pVideoCompressorParam,  int  iIndex_sharedObj  )
{
	...
	VIDEO_COMPRESSOR_CFG* pVideoCompressor; pVideoCompressor = &pCompressVideo->compressor;
	...
	//
			...
			BOOL	bAudio; bAudio = FALSE;	//  CAP_STUFF_bAudio(  *ppCapStuff  );
	BOOL	bVideo; bVideo = TRUE;	//  CAP_STUFF_bVideo(  *ppCapStuff  );
 ...
 int  makeSlaveStream(  QY_MC  *  pQyMc,  HWND  hDlg,  int  iIndex_sharedObj_master,  int  level,  int  iFourcc_suggested  )
{
	...
	//
	int  len; len = sizeof(SHARE_dyn_bmp);
	...
	//	
	int iTaskId_suggested; iTaskId_suggested = 0;
	{
		void * pDb=NULL;
		CQnmDb db;
		if(!db.getAvailableDb(pQyMc->iDsnIndex_mainSys)) goto errLabel;
	...
	int  iIndex_sharedObj_slave; iIndex_sharedObj_slave = pShareDynBmp_slave->var.iIndex_sharedObj;

	//
	QY_SHARED_OBJ* pSharedObj_slave; pSharedObj_slave = getSharedObjByIndex(pProcInfo, iIndex_sharedObj_slave);
	if  (  !pSharedObj_slave  )  goto  errLabel;
