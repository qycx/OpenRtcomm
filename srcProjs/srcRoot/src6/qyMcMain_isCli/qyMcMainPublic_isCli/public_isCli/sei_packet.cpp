...
//
int fill_sei_packet_h264(unsigned char* packet, int  *  piPktLen,  bool isAnnexb, const char* content, uint32_t size)
{
	if (!piPktLen)  return  -1;
...
//
int fill_sei_packet_h265(unsigned char* packet, int* piPktLen, bool isAnnexb, const char* content, uint32_t size)
{
	if (!piPktLen)  return  -1;
