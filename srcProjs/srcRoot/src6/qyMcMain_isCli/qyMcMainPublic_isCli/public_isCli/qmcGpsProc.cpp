 ...
 int  talkerShadow_procGps(  void  *  pProcInfoParam,  HWND  hDlgTalk,  BOOL  bForceRefreshPos,  int  index_shadow,  HWND  hDlgDynBmps_param,  void  *  pDlgDynBmpsVar_param,  int  index_wallGpsMgrMem  )
{
	...
	HWND	hWall_gps; hWall_gps = pShadowMgr->shadows[index_shadow].hWall;
	...
	DLG_dynBmps_var* pDlgDynBmpsVar; pDlgDynBmpsVar = NULL;
	...
	CQyVWall* pWall; pWall = pDlgDynBmpsVar->pWall;
	if  (  !pWall  )  goto  errLabel;
	SUB_V_WALL* pSubWall; pSubWall = pWall->getCurSubWall();
	if  (  !pSubWall  )  goto  errLabel;
	SP_STRUCT* pSp; pSp = (SP_STRUCT*)pSubWall->pSp;
	if  (  !pSp  )  goto  errLabel;
	if  (  !pSp->spWidth  ||  !pSp->spHeight  )  goto  errLabel;

	//
	BOOL	bSizeChanged; bSizeChanged = FALSE;
	int		x, y;
	x = 0, y = 0;
	int		iW, iH;
	iW = 0, iH = 0;
	...
	//
	float  fx; fx = 1. * pSubWall->iW_image / pSp->spWidth;
	float  fy; fy = 1. * pSubWall->iH_image / pSp->spHeight;
	...
	//  2012/05/06
	BOOL	bAdjusted; bAdjusted = FALSE;
	...
	UINT	uFlags; uFlags = SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOZORDER;
