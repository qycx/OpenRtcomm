...
int  procMsgInput_mgr_resp(  void  *  pQyMcParam,  void  *  pSgiParam,  MC_VAR_isCli  *  pProcInfo,  MIS_MSG_INPUT  *  pMsgInput,  unsigned  int  lenInBytes_msg,  void  *  pCTX_MC_THREAD,  MIS_CHANNEL  *  pChannel  )
{
	 ...
	 switch  (  pContent->uiType  )  {
			 ...
			 case  CONST_imCommType_refreshRecentFriendsResp:  				   
				   ...
				   //
#ifdef  __DEBUG__
				   traceLog((TCHAR*)_T("recentFriendsResp: cnt %d"),  pContent->refreshRecentFriendsResp.usCnt);
				   int  i;
				   for (i = 0; i < pContent->refreshRecentFriendsResp.usCnt; i++) {
					   REFRESH_RECENTFRIEND_RESP_h* pMem = &pContent->refreshRecentFriendsResp.mems[i];
					   //
					   traceLog((TCHAR*)_T("idInfo %I64u: conf_id %I64u"), pMem->idInfo.ui64Id, pMem->conf_ui64Id);
				   }
#endif 
