...
// 加载 DLL
 ...
 //
 bool  bUse_icCapTest(TCHAR  *  smCfgFile  )
 {
	 bool  bRet = false;
	 ...
	 //USB Video Device
	 getCfgValByNameT(smCfgFile, (TCHAR*)CONST_cfgName_icCapTest, cfgVal, mycountof(cfgVal));
	 tTrim(cfgVal);
...
#define		CONST_cfgName_icCap				_T(  "icCap")

	 TCHAR  cfgVal[128];
	 cfgVal[0] = 0;

	 //USB Video Device
	 getCfgValByNameT(smCfgFile, (TCHAR*)CONST_cfgName_icCap, cfgVal, mycountof(cfgVal));
	 tTrim(cfgVal);
	 int  tmpiRet = _ttol(cfgVal);
 ...
 extern "C" DWORD WINAPI threadProc_ic_cap_test(LPVOID lpParameter)
 {
	 SHARE_IC* p = (SHARE_IC*)lpParameter;
	 int  size = tmp_iW * tmp_iH * 3;
	 CQyMalloc  mallocObj;
	 ...
	 for (;;) {
		 ...
		 //
		 if (0) {
			 traceLog((TCHAR*)_T("ic_cap_test: loopCtrl %d"), loopCtrl);
			 if (loopCtrl > 1000) {
				 //
				 traceLog((TCHAR*)_T("ic_cap_test: ready to quit"));
				 //
				 exit(0);
			 }
		 }
 ...
 int  initShareIc(  void  **  ppCapStuff,  AUDIO_COMPRESSOR_CFG  *  pAudioCompressor,  BITMAPINFOHEADER  *  pBih_suggested,  HWND  hWnd_notify,  int  index_sharedObj,  void  **  ppShareMediaDeviceParam  )
{
	...
	//
	HINSTANCE hDll; hDll = LoadLibrary(L"hikIC.dll");
	if (!hDll) return -1;	

	GetImageSizeFunc getSizeFunc; getSizeFunc = (GetImageSizeFunc)GetProcAddress(hDll, "GetImageSize");
	if (getSizeFunc != NULL) {
	...
	//
	CCtxQyMc* pQyMc; pQyMc = g_pQyMc;
	

	//
	LPTHREAD_START_ROUTINE lpStartAddress;
	lpStartAddress = threadProc_ic_cap;
	if (bUse_icCapTest(pQyMc->cfg.smCfgFile)) {
		lpStartAddress = threadProc_ic_cap_test;
		//
...
//#ifdef  __DEBUG__
	...
	p->hThread_cap = CreateThread(NULL, 0, lpStartAddress, p, CREATE_SUSPENDED, &dwThreadDaemonId);
