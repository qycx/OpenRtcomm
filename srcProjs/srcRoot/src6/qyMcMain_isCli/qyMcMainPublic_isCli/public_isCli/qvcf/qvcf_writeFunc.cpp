...
int  qvcf_write_streamInfo(  VCF_streamInfo_common  *  p,  char  *  buf,  unsigned  int  *puiSize,  FILE  *  fp  )
{
	...
	unsigned  int  size_left_lineHeaderData; size_left_lineHeaderData = *puiSize - CONST_vcfLineCommon_len;
	...
	int  lineHeaderData_len; lineHeaderData_len = *puiSize - CONST_vcfLineCommon_len - size_left_lineHeaderData;
	//
	ptr  =  buf;
	...
	//
	int  len; len = lineHeaderData_len + CONST_vcfLineCommon_len;
	fwrite(  buf,  len,  1,  fp  );
