...
{	
	...
	//
	EVT_cli* pEvtCli; pEvtCli = pObj;
	//
	TCHAR  tBuf[128]; tBuf[0] = 0;// = _T("");
	...
	//
	int  iIndex_capProcInfo; iIndex_capProcInfo = 0;
	...
	// create pipe
	//
	unsigned  int  uiRtspType; uiRtspType = CONST_rtspType_encV;
	...
	//
	int  iFourcc_enc; iFourcc_enc = fourccStr2i(pCompressor->fourccStr);
	...
	//
	int  iIndex_sharedObj; iIndex_sharedObj = -1;
	...
	//
	QY_shm* pShm; pShm = &pEvtCli->dataShm;
	//
	if  (  pShm->hMap   )  {
		goto  errLabel;						  
	}
							...
							//
	unsigned  char  ucCnt_shmPktBufs; ucCnt_shmPktBufs = MAX_shmPktBufs_evt;
...
int  CMySmplYUVReader_myLoadNextFrame_evtCli(  unsigned  short  usPktResType,  void  *  p0_pInput,  void  *  p1_uiInputSize,  void  *  p2_myLoadNextFrame  )		
{
	...
	//
	 ...
	 //
	 int  index_toWrite; index_toWrite = pShmContent->status.writeShm.uiCnt_writeShm % ucCnt_shmPkts;
	 VT_shm_pktInfo* pShmPkt; pShmPkt = &pShmContent->mems[index_toWrite];
