 ...
 UINT  CSmsTraffic::SmThread(LPVOID lParam)
{
	...
	//	
	CString	m_strPort,  m_strRate,  m_strSmsc;
	HANDLE  hComm		=	NULL;					//  2008/01/04
 
	m_strPort  =  "com1";
	//  m_strRate=GetProfileString("Settings", "Rate");
	m_strRate  =  "9600";
	//  m_strSmsc=GetProfileString("Settings", "SMSC");
	m_strSmsc  =  "+8613800100500";
...
//				TRACE("State=stSendMessageResponse\n");
				Sleep(100);
				tmNow = CTime::GetCurrentTime();
				switch (gsmGetResponse(  hComm,  &buff))
				{
					case GSM_OK: 
//						TRACE("  GSM_OK %d\n", tmNow - tmOrg);
						nState = stBeginRest;
...
//				TRACE("State=stDeleteMessageRequest\n");
				if (nDelete < nMsg)
				{
					gsmDeleteMessage(  hComm,  param[nDelete].index);
					memset(&buff, 0, sizeof(buff));
					tmOrg = CTime::GetCurrentTime();
