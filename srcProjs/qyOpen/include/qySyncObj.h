...
class  CQySyncObj  {
 ...
 public:
		...
		int  sync(  LPCTSTR  mutexName,  unsigned  int  uiMilliSeconds  =  DEFAULT_milliseconds_syncQmObjQ  )  {  
				if  (  m_var.hMutex  )  return  -1;
				m_var.hMutex  =  syncQmObjQ(  mutexName,  uiMilliSeconds  );  
				return  m_var.hMutex  ?  0  :  -1;  
		};
...
///////////
typedef  struct  __qyMemory_t	{
				 int			iAllocType;
				 unsigned  int	uiBufSize;				//  2011/12/04
