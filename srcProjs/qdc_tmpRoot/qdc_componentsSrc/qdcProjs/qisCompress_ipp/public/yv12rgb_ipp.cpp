...
#define  __USE_ippYuv__
#define  __USE_YCbCr__
 ...
 int  yuy2ToYv12_ipp(  char  *  inBuff,  char  *  outBuf,  int  width,  int  height  )
{
	...
	unsigned  char  *  yuv  =  (  unsigned  char  *  )outBuf;
	...
	ipp_status  =  ippiYCbCr422ToYCrCb420_8u_C2P3R(  (  Ipp8u  *  )inBuff, srcStep, dst, pDstStepYVU, isize);
	if  (  ipp_status  )  return  -1;
...
int  yuy2ToRgb24_ipp(  unsigned  char  *  yuv,  unsigned  char  *  rgb,  int  width,  int  height,  BOOL  bFlipImg  )
{
	...
	ipp_status = ippiYCbCr422ToBGR_8u_C2C3R(yuv,  srcStep, rgb, dstStep, isize);
	if  (  ipp_status  )  return  -1;

	if  (  !bFlipImg  )  {
		unsigned  int  rgbIncrement  =  3;
		ippiMirror_8u_C3IR(  rgb, width * rgbIncrement,  isize,ippAxsHorizontal); 
	}
...
int  uyvyToRgb24_ipp(  unsigned  char  *  yuv,  unsigned  char  *  rgb,  int  width,  int  height,  BOOL  bFlipImg  )
{
	...
	//  tmp_showInfo(  _T(  "uyvyToRgb24_ipp enters"  )  );

	int  srcStep  =  2  *  width;
	int  dstStep  =  3  *  width;	

	ipp_status = ippiCbYCr422ToBGR_709HDTV_8u_C2C3R(yuv,  srcStep, rgb, dstStep, isize);
	if  (  ipp_status  )  return  -1;
	...
	//  tmp_showInfo(  _T(  "uyvyToRgb24_ipp leaves"  )  );
	return  0;
}
...
#if  0
 int  i420ToRgb24( unsigned  char  *  yuv,  unsigned  char  *  rgb,  int  width,  int  height  )
{
	...
	yuv420ToRgb24(  y_src,  u_src,  v_src,  rgb,  width,  height  );
 ...
 int  rgb24ToYv12(  unsigned  char  *  rgb,  unsigned  char  *  yuv,  int  width,  int  height,  BOOL  bFlip  )
{
	...
	for  (  y  =  0;  y  <  height;  y  ++  )  {
		 ...
		 if  (  bFlip  )  rgbIndex  =  rgb  +  width  *  (  height  -  1  -  y  )  *  rgbIncrement;
  ...
  int  rgb24ToYv12_ipp(  unsigned  char  *  rgb,  unsigned  char  *  yuv,  int  width,  int  height,  BOOL  bFlipImg  )
{
...
#if  1	//  def  __USE_ippYuv__				//  2011/09/13
  ...
  //
  //  这里将图像翻转,否则图像成倒立了. 2014/05/24
  if  (  !bFlipImg  )  {  
	  ippiMirror_8u_C3IR(  rgb, width * rgbIncrement,  isize,ippAxsHorizontal); 
  }


#ifndef  __USE_YCbCr__
  ipp_status = ippiBGRToYUV420_8u_C3P3(rgb, dst, isize);
  if (ipp_status!=0) {
	  //  printf("IPP Error: %d rgb2yuv\n",ipp_status);
	  OutputDebugString(  _T(  "IPP Error\n"  )  );
	  return  -1;
  }
#else
  ...
  ipp_status = ippiBGRToYCbCr420_709CSC_8u_C3P3R(rgb, srcStep, dst, pDstStepYVU, isize);
  if (ipp_status!=0) {
	  //  printf("IPP Error: %d rgb2yuv\n",ipp_status);
	  OutputDebugString(  _T(  "IPP Error\n"  )  );
	  return  -1;
  ...
  int  yv12ToRgb24_ipp( unsigned  char  *  yuv,  unsigned  char  *  rgb,  int  width,  int  height,  BOOL  bFlipImg  )
{
  ...
  ipp_status = ippiYUV420ToBGR_8u_P3C3( src, rgb, isize);
  if (ipp_status!=0) {
	  //  printf("IPP Error: %d rgb2yuv\n",ipp_status);
	  OutputDebugString(  _T(  "IPP Error\n"  )  );
	  return  -1;
  ...
  int pSrcStepYVU[3] = {width, width/2, width/2}; 

 
  ipp_status = ippiYCbCr420ToBGR_709CSC_8u_P3C3R((const Ipp8u  **  )src,  pSrcStepYVU, rgb, dstStep, isize);
 if (ipp_status!=0) {
	  //  printf("IPP Error: %d rgb2yuv\n",ipp_status);
	  OutputDebugString(  _T(  "IPP Error\n"  )  );
	  return  -1;
