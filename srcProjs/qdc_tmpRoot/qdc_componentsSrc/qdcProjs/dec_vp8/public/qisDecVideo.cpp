...
extern  "C"  int  __declspec(  dllexport  )  qdcDecompressVideoRun(  QDC_OBJ_INFO  *  pQdcObjInfo  )
{
	...
	memset(  pObj->yuvWriter.m_var.pPkt,  0,  pObj->yuvWriter.m_var.size_pPkt  );  //  2016/12/14
#endif
	...
	for  (  ;  ;  )  {
		...
		//
		if  (  pQdcObjInfo->cfg.v.dec.pf_smplYUVWriter_WriteNextFrame_all(  pQdcObjInfo,  &pObj->common,  iFourcc,  &pObj->yuvWriter.m_var,  0,  CropW,  CropH,  0,  my_surface_to_yuv,  dFrameRate,  _T( ""  )  )  )  goto  errLabel;
...
errLabel:
	...
	if  (  pObj->yuvWriter.m_var.pPkt  )  {
		//  2016/12/13
		pQdcObjInfo->cfg.v.pf_clean_myDRAW_VIDEO_DATA(  pObj->yuvWriter.m_var.pPkt,  _T(  ""  )  );
		//
