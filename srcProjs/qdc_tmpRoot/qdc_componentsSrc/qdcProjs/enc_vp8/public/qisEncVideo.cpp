...
//
int  CMySmplYUVReader_myLoadNextFrame(  unsigned  short  usPktResType,  char  *  pInput,  unsigned  int  uiInputSize,  PARAM_CMySmplYUVReader_myLoadNextFrame  *  pParam  )
{
	int  iErr  =  -1;
...
extern  "C"  int  __declspec(  dllexport  )  qdcCompressVideoRun(  QDC_OBJ_INFO  *  pQdcObjInfo  )
{
	...
	for  (  ;  ;  )  {
		 ...
		 {	 
			 ...
			 //
			 pQdcObjInfo->cfg.v.enc.pf_doPostEnc(  pQdcObjInfo,  pInput,  inputSize,  iSampleTimeInMs,  uiPts,  pObj->bsWriter.pPkt  );		  
